...
/**
 * Haftalık ziyaretçi soru özeti — son 7 günün en çok sorulan 5 sorusu, konu
 * dağılımı (geçen haftayla karşılaştırma) ve yanıtlanamayan/yönlendirme oranı
 * e-posta ile yönetime gönderilir (haftada bir kez, idempotent).
 *
 * Zamanlayıcı: nexus-chat-weekly (varsayılan 0 8 * * 1 — pazartesi 08:00) — admin → Zamanlayıcılar.
...
if ($to === '' || !filter_var($to, FILTER_VALIDATE_EMAIL)) {
    fwrite(STDERR, "Haftalık özet gönderilmedi: admin_alert_email tanımlı değil (admin → Kontrol merkezi).\n");
    exit(0);
}

// İdempotent: ISO hafta anahtarı (örn. 202633) — aynı hafta yalnızca bir kez kuyruğa eklenir.
$weekKey = (int) date('oW');
...
if ((int) $exists->fetchColumn() > 0) {
    echo "Bu hafta için özet zaten kuyrukta; atlandı.\n";
    exit(0);
}
...
// Kalitesiz girdiler özetten çıkarılır (eşikler admin → Kontrol merkezi'nde).
...
// Yönlendirilen yanıtlar (sayfa bağlantısı / red cümlesi) ve reddedilen istekler.
...
// Konu dağılımı: bu hafta vs geçen hafta (ortak sınıflandırıcı).
...
if ($total === 0 && $denied === 0) {
    echo "Son 7 günde soru yok; e-posta gönderilmedi.\n";
    exit(0);
}
...
$body = '<div style="font-family:Arial,sans-serif;color:#10211f">'
    ...
    . ($rows !== '' ? '<table style="border-collapse:collapse;width:100%;max-width:560px">'
...
echo 'Haftalık özet kuyruğa eklendi: ' . $to . ' (' . count($top) . " soru, toplam {$total}).\n";
