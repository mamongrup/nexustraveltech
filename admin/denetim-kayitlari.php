...
<div class="chips"><a class="chip <?=$action===''?'on':''?>" href="<?=$baseAudit?><?= $extra !== '' ? '?' . $extra : '' ?>">Tümü</a><a class="chip <?=$action==='feature.restore'?'on':''?>" style="background:<?=$action==='feature.restore'?'#405b13':'#e6f8c7'?>;color:#10211f;border-color:#bcd98a" href="?action=<?=urlencode('feature.restore')?><?=$filtersSuffix?>">↩ Geri alınan özellikler</a><a class="chip <?=$action==='feature.delete'?'on':''?>" style="background:<?=$action==='feature.delete'?'#8e2410':'#ffe2de'?>;color:#10211f;border-color:#f3c4ba" href="?action=<?=urlencode('feature.delete')?><?=$filtersSuffix?>">✗ Silinen özellikler</a><a class="chip <?=$action==='feature.trash_purge'?'on':''?>" href="?action=<?=urlencode('feature.trash_purge')?><?=$filtersSuffix?>">🗑 Çöp temizliği</a><a class="chip <?=str_starts_with($action,'feature.bulk_')?'on':''?>" style="background:<?=str_starts_with($action,'feature.bulk_')?'#2b4a7a':'#eef3fb'?>;color:#10211f;border-color:#b9cbe8" href="?action=<?=urlencode('feature.bulk_')?><?=$filtersSuffix?>">⚡ Toplu işlemler</a><a class="chip <?=$action==='channel.room_mapping_approve'?'on':''?>" style="background:<?=$action==='channel.room_mapping_approve'?'#0d7a4a':'#e6f4ee'?>;color:#10211f;border-color:#b5dcc9" href="?action=<?=urlencode('channel.room_mapping_approve')?><?=$filtersSuffix?>">🛏 Oda eşleştirme onayı<?= ($actionCounts['channel.room_mapping_approve'] ?? 0) > 0 ? '<small style="background:rgba(0,0,0,.12);border-radius:8px;padding:1px 6px;margin-left:4px;font-size:10px;font-weight:bold">' . (int)$actionCounts['channel.room_mapping_approve'] . '</small>' : '' ?></a><a class="chip <?=$action==='channel.room_mapping_reject'?'on':''?>" style="background:<?=$action==='channel.room_mapping_reject'?'#8e2410':'#ffe2de'?>;color:#10211f;border-color:#f3c4ba" href="?action=<?=urlencode('channel.room_mapping_reject')?><?=$filtersSuffix?>">🛏 Oda eşleştirme reddi<?= ($actionCounts['channel.room_mapping_reject'] ?? 0) > 0 ? '<small style="background:rgba(0,0,0,.12);border-radius:8px;padding:1px 6px;margin-left:4px;font-size:10px;font-weight:bold">' . (int)$actionCounts['channel.room_mapping_reject'] . '</small>' : '' ?></a><a class="chip <?=$action==='channel.plan_mapping_approve'?'on':''?>" style="background:<?=$action==='channel.plan_mapping_approve'?'#0d7a4a':'#e6f4ee'?>;color:#10211f;border-color:#b5dcc9" href="?action=<?=urlencode('channel.plan_mapping_approve')?><?=$filtersSuffix?>">💶 Plan eşleştirme onayı<?= ($actionCounts['channel.plan_mapping_approve'] ?? 0) > 0 ? '<small style="background:rgba(0,0,0,.12);border-radius:8px;padding:1px 6px;margin-left:4px;font-size:10px;font-weight:bold">' . (int)$actionCounts['channel.plan_mapping_approve'] . '</small>' : '' ?></a><a class="chip <?=$action==='channel.plan_mapping_reject'?'on':''?>" style="background:<?=$action==='channel.plan_mapping_reject'?'#8e2410':'#ffe2de'?>;color:#10211f;border-color:#f3c4ba" href="?action=<?=urlencode('channel.plan_mapping_reject')?><?=$filtersSuffix?>">💶 Plan eşleştirme reddi<?= ($actionCounts['channel.plan_mapping_reject'] ?? 0) > 0 ? '<small style="background:rgba(0,0,0,.12);border-radius:8px;padding:1px 6px;margin-left:4px;font-size:10px;font-weight:bold">' . (int)$actionCounts['channel.plan_mapping_reject'] . '</small>' : '' ?></a><a class="chip <?=$action==='channel.token_reset'?'on':''?>" style="background:<?=$action==='channel.token_reset'?'#8a6100':'#fdf9f2'?>;color:#10211f;border-color:#e0c9a3" href="?action=<?=urlencode('channel.token_reset')?><?=$filtersSuffix?>">🔑 Token sıfırlama<?= ($actionCounts['channel.token_reset'] ?? 0) > 0 ? '<small style="background:rgba(0,0,0,.12);border-radius:8px;padding:1px 6px;margin-left:4px;font-size:10px;font-weight:bold">' . (int)$actionCounts['channel.token_reset'] . '</small>' : '' ?></a><?php foreach ($actions as $a): ?><a class="chip <?=$action===$a['action']?'on':''?>" href="?action=<?=urlencode($a['action'])?><?=$filtersSuffix?>"><?=htmlspecialchars($a['action'])?> (<?=(int)$a['c']?>)</a><?php endforeach; ?></div>
<table><tr><th>Zaman</th><th>Yönetici</th><th>Tedarikçi</th><th>İşlem</th><th>Nesne</th><th>Detay</th><th>IP</th></tr>
