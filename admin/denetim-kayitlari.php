...
<div class="chips"><a class="chip <?=$action===''?'on':''?>" href="/nexustraveltech/admin/denetim-kayitlari">Tümü</a><a class="chip <?=$action==='feature.restore'?'on':''?>" style="background:<?=$action==='feature.restore'?'#405b13':'#e6f8c7'?>;color:#10211f;border-color:#bcd98a" href="?action=<?=urlencode('feature.restore')?>">↩ Geri alınan özellikler</a><a class="chip <?=$action==='feature.delete'?'on':''?>" style="background:<?=$action==='feature.delete'?'#8e2410':'#ffe2de'?>;color:#10211f;border-color:#f3c4ba" href="?action=<?=urlencode('feature.delete')?>">✗ Silinen özellikler</a><a class="chip <?=$action==='feature.trash_purge'?'on':''?>" href="?action=<?=urlencode('feature.trash_purge')?>">🗑 Çöp temizliği</a><?php foreach ($actions as $a): ?><a class="chip <?=$action===$a['action']?'on':''?>" href="?action=<?=urlencode($a['action'])?>"><?=htmlspecialchars($a['action'])?> (<?=(int)$a['c']?>)</a><?php endforeach; ?></div>
<table><tr><th>Zaman</th><th>Yönetici</th><th>İşlem</th><th>Nesne</th><th>Detay</th><th>IP</th></tr>
<?php foreach ($rows as $r): $details = json_decode((string)$r['details'], true); $isRestore = $r['action'] === 'feature.restore'; ?>
<tr<?= $isRestore ? ' style="background:#f4fbea"' : '' ?>><td><?=htmlspecialchars((string)$r['created_at'])?></td><td><?=htmlspecialchars($r['admin_username'])?></td><td><code><?=htmlspecialchars($r['action'])?></code></td><td><?=htmlspecialchars((string)($r['entity_type'] ?? ''))?><?= $r['entity_id'] ? ' #' . (int)$r['entity_id'] : '' ?></td><td><?php if ($details): ?><pre><?=htmlspecialchars(json_encode($details, JSON_UNESCAPED_UNICODE))?></pre><?php endif; ?></td><td><?=htmlspecialchars((string)$r['ip'])?></td></tr>
<?php endforeach; ?>
</table>
