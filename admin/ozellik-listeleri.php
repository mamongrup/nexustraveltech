...
if (in_array($export, ['delete_impact', 'delete_type', 'bulk_impact', 'bulk_type'], true) && $_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if ($export === 'delete_type') {
        ...
        header('Content-Disposition: attachment; filename="etki-tur-tablosu.csv"');
        $out = fopen('php://output', 'w');
        fwrite($out, "\xEF\xBB\xBF"); // BOM — Excel'de Türkçe karakterler
        fputcsv($out, ['Tür', 'İlan sayısı']);
        foreach (['hotel' => 'Otel', 'villa' => 'Villa', 'yacht' => 'Yat', 'total' => 'Toplam'] as $k => $tl) {
            fputcsv($out, [$tl, (int) $byType[$k]]);
        }
        fclose($out);
        exit;
    }
    ...
    try {
      ...
      if ($action === 'add') {
        ...
        if ($sub === 'delete' && empty($_POST['confirm'])) {
          ...
          setcookie('nexus_bulk_result', $msg, [
              'expires' => time() + 3600,
              'path' => '/',
              'httponly' => false,
              'samesite' => 'Lax',
...
function toggleCat(code){var b=document.getElementById('catBody-'+code),a=document.getElementById('catArrow-'+code);if(!b)return;var hidden=b.style.display==='none';b.style.display=hidden?'block':'none';a.textContent=hidden?'▾ daralt':'▸ genişlet'}
function collapseAll(open){document.querySelectorAll('[id^="catBody-"]').forEach(function(b){b.style.display=open?'block':'none';var a=document.getElementById('catArrow-'+b.id.replace('catBody-','catArrow-'));if(a)a.textContent=open?'▾ daralt':'▸ genişlet'})}
