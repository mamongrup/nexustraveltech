...
// Tekil işlem (silme/geri yükleme/sıralama) sonucunu çereze yaz — sayfa yenilense bile
// kalıcı bildirim olarak görünür (toplu işlemdeki nexus_bulk_result ile aynı desen).
$setActionCookie = function (string $sub, string $msg, array $extra = []): void {
    setcookie('nexus_action_result', json_encode(array_merge(['msg' => $msg, 'sub' => $sub], $extra), JSON_UNESCAPED_UNICODE), [
        'expires' => time() + 3600,
        'path' => '/',
        'httponly' => false,
        'samesite' => 'Lax',
...
$restoreFeature = function (int $featureId): array {
    ...
    db()->prepare('UPDATE property_feature_catalog SET deleted_at=NULL, previous_purge_at=purge_at, purge_at=NULL, group_label=?, label=?, sort_order=?, is_active=? WHERE id=?')
...
if (($_GET['qview'] ?? '') === 'trash_listings' && $_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    try {
        ...
        // Ozet: toplam fiyat ve bolum dagilimi
        $totalPrice = 0;
        $secDist = [];
        foreach ($list as $li) {
            $pVal = preg_replace('/[^0-9.,]/', '', (string) ($li['price'] ?? ''));
            $pVal = str_replace(',', '.', $pVal);
            if (is_numeric($pVal) && $pVal > 0) $totalPrice += (float) $pVal;
            foreach ($li['sections'] as $sec) {
                $secDist[$sec] = ($secDist[$sec] ?? 0) + 1;
            }
        }
        arsort($secDist);
        // property_type'ı DB'den çek (link oluşturma için)
        $pidList = array_map(fn($li) => (int) $li['id'], $list);
        if ($pidList) {
            $ph = str_repeat('?,', count($pidList) - 1) . '?';
            $ptq = db()->prepare("SELECT id, property_type FROM properties WHERE id IN ($ph)");
            $ptq->execute($pidList);
            $ptMap = [];
            foreach ($ptq->fetchAll() as $ptRow) $ptMap[(int) $ptRow['id']] = (string) $ptRow['property_type'];
            foreach ($list as &$li) { $li['property_type'] = $ptMap[$li['id']] ?? ''; }
            unset($li);
        }
        exit(json_encode(['ok' => true, 'label' => (string) $bk['label'], 'listings' => $list, 'summary' => ['total_price' => $totalPrice, 'section_dist' => $secDist, 'count' => count($list)]], JSON_UNESCAPED_UNICODE));
    } catch (Throwable $e) {
        http_response_code(500);
        exit(json_encode(['ok' => false, 'error' => $e->getMessage()], JSON_UNESCAPED_UNICODE));
    }
}
// Seçili ilanlara geri yükleme — önizleme kutusundaki tek ilan bazlı geri ekleme.
if (($_GET['qview'] ?? '') === 'trash_restore_listings' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    header('Content-Type: application/json; charset=UTF-8');
    $fid = (int) ($_GET['id'] ?? 0);
    $listingIds = array_values(array_unique(array_filter(array_map('intval', (array) ($_POST['listing_ids'] ?? [])), fn($i) => $i > 0)));
    if ($fid <= 0) { http_response_code(400); exit(json_encode(['ok' => false, 'error' => 'Gecersiz ozellik kimligi.'])); }
    if (!$listingIds) { http_response_code(400); exit(json_encode(['ok' => false, 'error' => 'En az bir ilan secin.'])); }
    try {
        $bkQ = db()->prepare('SELECT * FROM feature_delete_backups WHERE feature_id=? ORDER BY id DESC LIMIT 1');
        $bkQ->execute([$fid]);
        $bk = $bkQ->fetch();
        if (!$bk) { http_response_code(404); exit(json_encode(['ok' => false, 'error' => 'Yedek kaydi bulunamadi.'])); }
        $featQ = db()->prepare('SELECT deleted_at FROM property_feature_catalog WHERE id=?');
        $featQ->execute([$fid]);
        $feat = $featQ->fetch();
        if (!$feat || $feat['deleted_at'] === null) { http_response_code(400); exit(json_encode(['ok' => false, 'error' => 'Ozellik su an cop kutusunda degil.'])); }
        $label = (string) $bk['label'];
        $allProps = json_decode((string) ($bk['affected_properties'] ?? '[]'), true) ?: [];
        $selectedProps = array_values(array_filter($allProps, fn($p) => in_array((int) ($p['id'] ?? 0), $listingIds, true)));
        if (!$selectedProps) { http_response_code(400); exit(json_encode(['ok' => false, 'error' => 'Secilen ilanlar yedekte bulunamadi.'])); }
        db()->prepare('UPDATE property_feature_catalog SET deleted_at=NULL, previous_purge_at=purge_at, purge_at=NULL WHERE id=?')->execute([$fid]);
        db()->prepare('DELETE FROM pending_trash_purges WHERE feature_id=?')->execute([$fid]);
        $restored = 0;
        foreach ($selectedProps as $pr) {
            $sections = is_array($pr['sections'] ?? null) ? $pr['sections'] : [];
            $pid = (int) ($pr['id'] ?? 0);
            $price = (string) ($pr['price'] ?? '');
            foreach ($sections as $sec) {
                if ($sec === 'service_pricing') {
                    db()->prepare("UPDATE properties SET product_details = jsonb_set(product_details, '{service_pricing}', COALESCE(product_details -> 'service_pricing', '{}'::jsonb) || jsonb_build_object(?, ?), true) WHERE id=? AND NOT jsonb_exists(COALESCE(product_details -> 'service_pricing', '{}'::jsonb), ?)")
                        ->execute([$label, $price, $pid, $label]);
                } elseif (in_array($sec, ['amenities', 'activities', 'events'], true)) {
                    db()->prepare("UPDATE properties SET product_details = jsonb_set(product_details, '" . $sec . "', COALESCE(product_details -> '" . $sec . "', '[]'::jsonb) || ?::jsonb, true) WHERE id=? AND NOT (COALESCE(product_details -> '" . $sec . "', '[]'::jsonb) @> ?::jsonb)")
                        ->execute([json_encode([$label]), $pid, json_encode([$label])]);
                }
                $restored++;
            }
        }
        audit_log('feature.restore_partial', 'feature_catalog', $fid, [
            'code' => $bk['code'] ?? '',
            'label' => $label,
            'affected_count' => count($selectedProps),
            'listing_ids' => $listingIds,
            'restored_sections' => $restored,
        ]);
        exit(json_encode(['ok' => true, 'label' => $label, 'restored' => count($selectedProps), 'sections' => $restored], JSON_UNESCAPED_UNICODE));
    } catch (Throwable $e) {
        http_response_code(500);
        exit(json_encode(['ok' => false, 'error' => $e->getMessage()], JSON_UNESCAPED_UNICODE));
    }
...
// Metrik detay listesi — ilan bazında görsel/oda/plan/rezervasyon sayıları.
// Hem rozet tıklama uç noktası (qview) hem ayrıntılı metrik CSV'si ortak kullanır.
$metricDetailList = function (array $ids): array {
...
if (($_GET['qview'] ?? '') === 'bulk_metrics' && $_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    try {
        exit(json_encode(['ok' => true, 'list' => $metricDetailList($ids)], JSON_UNESCAPED_UNICODE));
    } catch (Throwable $e) {
        http_response_code(500);
        exit(json_encode(['ok' => false, 'error' => $e->getMessage()], JSON_UNESCAPED_UNICODE));
    }
...
if (in_array($export, ['delete_impact', 'delete_type', 'bulk_impact', 'bulk_type', 'bulk_metrics', 'delete_supplier_summary', 'bulk_supplier_summary'], true) && $_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if ($export === 'delete_impact') {
        ...
        if ($export === 'bulk_metrics') {
            $detail = (($_GET['detail'] ?? '') === '1');
            header('Content-Type: text/csv; charset=UTF-8');
            header('Content-Disposition: attachment; filename="' . ($detail ? 'etki-metrikler-detayli.csv' : 'etki-metrikler.csv') . '"');
            $out = fopen('php://output', 'w');
            fwrite($out, "\xEF\xBB\xBF"); // BOM — Excel'de Türkçe karakterler
            ...
            // Ayrıntılı mod — ilan başına satır (aynı ilan birden çok özellikle eşleşse bile tek satır).
            if ($detail) {
                $dl = $metricDetailList($ids);
                fputcsv($out, []);
                fputcsv($out, ['Tür', 'İlan adı', 'Tedarikçi', 'Görsel', 'Oda tipi', 'Fiyat planı', 'Rezervasyon', 'İlan ID']);
                foreach ($dl as $p) {
                    fputcsv($out, [$typeLabelCsv((string) $p['type']), (string) $p['name'], (string) $p['company'], (int) $p['media'], (int) $p['rooms'], (int) $p['plans'], (int) $p['bookings'], (int) $p['id']]);
                }
            }
            fclose($out);
            exit;
    ...
    try {
      ...
      if ($action === 'add') {
        ...
        if (empty($_POST['confirmed'])) {
          ...
          // Silme öncesi metrikler — etkilenen ilanların toplam görsel/oda/plan/rezervasyonu.
          $metrics = ['media' => 0, 'rooms' => 0, 'plans' => 0, 'bookings' => 0];
          foreach ($metricDetailList([$featureId]) as $md) {
              $metrics['media'] += (int) $md['media'];
              $metrics['rooms'] += (int) $md['rooms'];
              $metrics['plans'] += (int) $md['plans'];
              $metrics['bookings'] += (int) $md['bookings'];
          }
          $pendingDelete = ['id' => $featureId, 'label' => $feat['label'], 'affected' => $affected, 'by_type' => $byType, 'metrics' => $metrics];
        } else {
          ...
          $setActionCookie('delete', $msg, ['id' => $featureId, 'label' => $res['label'], 'affected' => count($res['affected'])]);
        }
        ...
        $setActionCookie('restore', $msg, ['label' => $res['label'], 'affected' => (int) $res['affected_count']]);
        ...
        $done = 0; $skipped = 0; $doneNames = []; $skippedNames = []; $skippedReasons = []; $errors = [];
        $curQ = db()->prepare('SELECT label, deleted_at FROM property_feature_catalog WHERE id=?');
        foreach ($ids as $fid) {
          try {
            $curQ->execute([$fid]);
            $cur = $curQ->fetch();
            if (!$cur) { $skipped++; $skippedNames[] = '#' . $fid; $skippedReasons[] = '#' . $fid . ': Kayıt bulunamadı (ID mevcut değil)'; continue; }
            if ($cur['deleted_at'] === null) { $skipped++; $skippedNames[] = (string) $cur['label']; $skippedReasons[] = (string) $cur['label'] . ': Çöp kutusunda değil (zaten aktif)'; continue; }
            $res = $restoreFeature($fid);
            $doneNames[] = (string) ($res['label'] ?? ('#' . $fid));
            $done++;
          } catch (Throwable $e) { $errors[] = '#' . $fid . ': ' . $e->getMessage(); }
        }
        audit_log('feature.bulk_restore', 'feature_catalog', null, ['count' => $done, 'feature_ids' => $ids, 'skipped_unchanged' => $skipped, 'done_names' => array_slice($doneNames, 0, 100), 'skipped_names' => array_slice($skippedNames, 0, 100), 'skipped_reasons' => array_slice($skippedReasons, 0, 20)]);
        ...
        setcookie('nexus_bulk_result', json_encode([
            ...
            'skipped_reasons' => array_slice($skippedReasons, 0, 12),
            'removed' => 0,
            'trash_ttl' => 0,
        ...
        if ($sub === 'delete' && empty($_POST['confirm'])) {
          ...
          $fq = db()->prepare('SELECT id, label, deleted_at FROM property_feature_catalog WHERE id=?');
          ...
          foreach ($ids as $fid) {
            ...
            $f = $fq->fetch();
            if (!$f) continue;
            // İdempotent önizleme: çöp kutusundaki seçili özellik uygulamada atlanacak — etki ve sayılara katılmaz.
            if ($f['deleted_at'] !== null) { $alreadyTrash[] = ['id' => (int) $fid, 'label' => (string) $f['label']]; continue; }
            $liveIds[] = (int) $fid;
            ...
            $items[] = ['label' => $f['label'], 'code' => (string) ($f['code'] ?? ''), 'affected_count' => count($aff), 'affected_names' => array_map(fn($a) => $a['name'], array_slice($aff, 0, 5))];
          ...
          $pendingBulk = ['ids' => $ids, 'items' => $items, 'total_affected' => $totalAffected, 'by_type' => $byType, 'metrics' => $metrics, 'already_trash' => $alreadyTrash];
        } else {
          // Adım 2: uygula (her özellik için tekil akışın aynısı + toplu denetim kaydı).
          $done = 0; $removed = 0; $skipped = 0; $doneNames = []; $skippedNames = []; $skippedReasons = []; $staleSkipped = 0; $errors = [];
          $expectedState = (array) ($_POST['expected_state'] ?? []);
          ...
          foreach ($ids as $fid) {
            try {
              if ($sub === 'deactivate' || $sub === 'activate') {
                ...
                // Stale-page koruması: sayfadaki beklenen durum ile DB'deki güncel durum karşılaştırılır.
                $expState = $expectedState[(string) $fid] ?? null;
                if ($expState !== null) {
                    $curState = (bool) $cur['is_active'] ? 'on' : 'off';
                    if ($curState !== $expState) {
                        $skipped++; $staleSkipped++;
                        $skippedNames[] = (string) $cur['label'];
                        $skippedReasons[] = (string) $cur['label'] . ': Sayfa yüklendiğinde ' . ($expState === 'on' ? 'aktif' : 'pasif') . ' idi, şimdi ' . ($curState === 'on' ? 'aktif' : 'pasif') . ' — başka bir yönetici değiştirmiş';
                        continue;
                    }
                }
                if ((bool) $cur['is_active'] === $newState) { $skipped++; $skippedNames[] = (string) $cur['label']; $skippedReasons[] = (string) $cur['label'] . ': Zaten ' . ($newState ? 'aktif' : 'pasif') . ' (idempotent)'; continue; }
                ...
                // Stale-page koruması: sayfadaki beklenen durum ile DB'deki güncel durum karşılaştırılır.
                $expState = $expectedState[(string) $fid] ?? null;
                if ($expState !== null && $cur['deleted_at'] === null && $expState === 'off') {
                    // Sayfada pasif görünüyordu ama aktif olmuş — stale sayfa
                }
                if ($expState !== null && $cur['deleted_at'] === null) {
                    $isCurrentlyDeleted = false;
                    if ($expState !== 'trash' && $cur['deleted_at'] !== null) {
                        $skipped++; $staleSkipped++;
                        $skippedNames[] = (string) $cur['label'];
                        $skippedReasons[] = (string) $cur['label'] . ': Sayfada mevcut görünüyordu ama artık çöp kutusunda — başka bir yönetici silmiş';
                        continue;
                    }
                }
                if ($cur['deleted_at'] !== null) { $skipped++; $skippedNames[] = (string) $cur['label']; $skippedReasons[] = (string) $cur['label'] . ': Zaten çöp kutusunda (idempotent)'; continue; }
          ...
          $auditDetails = [
              ...
              'skipped_reasons' => array_slice($skippedReasons, 0, 20),
              'stale_page_skipped' => $staleSkipped,
          ];
          if ($sub === 'delete') {
          ...
          $skipReason = $staleSkipped > 0
              ? ($staleSkipped . ' özellik sayfa eski olduğu için atlandı' . ($skipped > $staleSkipped ? ', ' . ($skipped - $staleSkipped) . ' özellik zaten hedef durumda' : ''))
              : ($sub === 'delete' ? 'zaten çöp kutusunda olduğu için atlandı (sayılmadı)' : ($sub === 'activate' ? 'zaten aktif olduğu için atlandı (sayılmadı)' : 'zaten pasif olduğu için atlandı (sayılmadı)'));
        ...
        if ($pos !== null && $target >= 0 && $target < count($rows) && $pos !== $target) {
          ...
          $setActionCookie('move', $msg, ['label' => $f['label'], 'direction' => $dir]);
        } else {
      ...
      } elseif ($action === 'resend_purge_email') {
        $featureId = (int) ($_POST['id'] ?? 0);
        if ($featureId <= 0) throw new RuntimeException('Geçersiz özellik.');
        $adminEmail = (string) platform_setting('admin_alert_email', '');
        if ($adminEmail === '') throw new RuntimeException('Admin e-posta adresi ayarlanmamış (kontrol merkezi).');
        $ttlDays = max(7, (int) platform_setting('feature_trash_ttl_days', 30));
        $fq = db()->prepare('SELECT id, code, label, deleted_at, purge_at FROM property_feature_catalog WHERE id=? AND deleted_at IS NOT NULL');
        $fq->execute([$featureId]);
        $feat = $fq->fetch();
        if (!$feat) throw new RuntimeException('Özellik çöp kutusunda bulunamadı.');
        // Mevcut pending kaydını bul veya yeni token üret
        $pendQ = db()->prepare('SELECT id, token, expires_at FROM pending_trash_purges WHERE feature_id=? AND approved_at IS NULL ORDER BY expires_at DESC LIMIT 1');
        $pendQ->execute([$featureId]);
        $pend = $pendQ->fetch();
        $token = $pend ? (string) $pend['token'] : bin2hex(random_bytes(32));
        if (!$pend) {
            $expiresAt = date('Y-m-d H:i:s', time() + 3 * 86400);
            db()->prepare('INSERT INTO pending_trash_purges (feature_id, token, expires_at) VALUES (?, ?, ?)')->execute([$featureId, $token, $expiresAt]);
        }
        $link = 'https://nexustraveltech.com/admin/approve-trash-purge.php?token=' . $token;
        $delTs = strtotime((string) $feat['deleted_at']) ?: time();
        $purgeTs = !empty($feat['purge_at']) ? (strtotime((string) $feat['purge_at']) ?: 0) : 0;
        if ($purgeTs <= 0) $purgeTs = $delTs + $ttlDays * 86400;
        $customTag = !empty($feat['purge_at']) ? ' · özel tarih' : '';
        $body = '<div style="font-family:Arial,sans-serif;color:#10211f">'
            . '<h2 style="margin:0 0 6px">⏳ Son şans: "' . htmlspecialchars($feat['label']) . '" kalıcı silinmek üzere</h2>'
            . '<p>Bu özellik silinme vadesini doldurdu. Onaylarsanız <b style="color:#9d3b1c">kalıcı olarak silinir ve geri alınamaz</b>.</p>'
            . '<p><b>Özellik:</b> ' . htmlspecialchars($feat['label']) . ' <span style="color:#6b7774">(' . htmlspecialchars((string) $feat['code']) . ' · silindi ' . htmlspecialchars(mb_substr((string) $feat['deleted_at'], 0, 10)) . ' · kalıcı silme ' . date('Y-m-d', $purgeTs) . $customTag . ')</span></p>'
            . '<p style="background:#fff3cd;border:1px solid #e0c9a3;border-radius:8px;padding:10px 12px"><a href="' . $link . '" style="color:#8a6100;font-weight:bold;font-size:15px;text-decoration:none">✅ Kalıcı silmeyi onayla →</a><br><span style="color:#6b7774;font-size:12px">Tek kullanımlık bağlantı, 3 gün geçerli.</span></p>'
            . '<p style="color:#6b7774;font-size:13px">Onaylamak istemiyorsanız bağlantıya tıklamayın — özellik çöp kutusunda kalır.</p>'
            . '<p><a href="https://nexustraveltech.com/admin/ozellik-listeleri#trash" style="color:#b0301a">Katalog & çöp kutusu →</a></p>'
            . '</div>';
        queue_email($adminEmail, 'Son şans: "' . $feat['label'] . '" kalıcı silinmek üzere (onay gerekli)', $body, 'trash_purge_approval', $featureId);
        $msg = 'Son şans e-postası yeniden gönderildi: ' . htmlspecialchars($feat['label']) . ' → ' . $adminEmail;
        $setActionCookie('resend', $msg, ['label' => $feat['label']]);
      } else {
...
$trashEnriched = [];
foreach ($trash as $t) {
    $_p = trash_effective_purge($t, $ttlDays);
    $t['_purge_ts'] = $_p['purge_ts'];
    $t['_remain']   = $_p['remain_days'];
    $t['_custom']   = $_p['custom'];
    $t['_urgent']   = $_p['remain_days'] < 7;
...
<?php $actRaw = (string) ($_COOKIE['nexus_action_result'] ?? ''); $actCard = null; $actMsg = ''; if ($actRaw !== '') { $decoded = json_decode($actRaw, true); if (is_array($decoded)) { $actCard = $decoded; $actMsg = (string) ($decoded['msg'] ?? ''); } else { $actMsg = trim($actRaw); } } ?>
<?php if ($msg): ?><p class="ok">✓ <?= htmlspecialchars($msg) ?></p><?php endif; ?>
<?php if ($bulkNotice !== '' || $bulkCard): ?><div id="bulkNotice" class="nexus-toast" data-ttl="8"><div class="nt-row"><div class="nt-body">⚡ <b>Toplu işlem sonucu:</b> <?= htmlspecialchars($bulkNotice) ?><?php if ($bulkCard): ?><div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:8px"><span style="display:inline-block;background:#2e7d32;color:#fff;border-radius:12px;padding:2px 10px;font-size:12px;font-weight:bold">✓ <?= (int) $bulkCard['done'] ?> değişti</span><?php if ((int) ($bulkCard['removed'] ?? 0) > 0): ?><span style="display:inline-block;background:#1a3d6d;color:#fff;border-radius:12px;padding:2px 10px;font-size:12px;font-weight:bold"><?= (int) $bulkCard['removed'] ?> ilan kaldırıldı</span><?php endif; ?><?php if ((int) ($bulkCard['skipped'] ?? 0) > 0): ?><span style="display:inline-block;background:#8a6100;color:#fff;border-radius:12px;padding:2px 10px;font-size:12px;font-weight:bold">⏭ <?= (int) $bulkCard['skipped'] ?> atlandı</span><?php endif; ?><?php if ((int) ($bulkCard['stale_skipped'] ?? 0) > 0): ?><span style="display:inline-block;background:#6b2d0e;color:#fdf3e3;border:1px solid #b26a00;border-radius:12px;padding:2px 10px;font-size:12px;font-weight:bold" title="Bu özellikler sayfa yüklendiğinden beri başka bir yönetici tarafından değiştirilmiş">🔄 <?= (int) $bulkCard['stale_skipped'] ?> sayfa eski</span><?php endif; ?><?php if (($bulkCard['sub'] ?? '') === 'delete'): ?><span style="display:inline-block;background:#8a6100;color:#fff;border-radius:12px;padding:2px 10px;font-size:12px;font-weight:bold" title="Silinen özellikler bu süre boyunca çöp kutusunda durur ve geri yüklenebilir">🗑 Çöp kutusunda <?= (int) ($bulkCard['trash_ttl'] ?? 30) ?> gün</span><?php endif; ?></div><?php if (!empty($bulkCard['done_names'])): ?><details style="margin-top:8px"><summary style="cursor:pointer;font-size:12px;color:#d9f0b4;font-weight:bold">Değişenler (<?= (int) $bulkCard['done'] ?>)</summary><ul style="margin:6px 0 0;padding-left:18px;font-size:12px;color:#e8f5d0"><?php foreach ($bulkCard['done_names'] as $dn): ?><li><?= htmlspecialchars((string) $dn) ?></li><?php endforeach; ?><?php if ((int) ($bulkCard['done_more'] ?? 0) > 0): ?><li style="color:#8a9aa0">… ve <?= (int) $bulkCard['done_more'] ?> daha</li><?php endif; ?></ul></details><?php endif; ?><?php if (!empty($bulkCard['skipped_names'])): ?><details style="margin-top:8px"><summary style="cursor:pointer;font-size:12px;color:#d9f0b4;font-weight:bold">Atlananlar (<?= (int) $bulkCard['skipped'] ?>)</summary><ul style="margin:6px 0 0;padding-left:18px;font-size:12px;color:#e8f5d0"><?php foreach ($bulkCard['skipped_names'] as $si => $sn): ?><li><?= htmlspecialchars((string) $sn) ?><?php if (!empty($bulkCard['skipped_reasons'][$si])): ?> <small style="color:#e0c9a3">— <?= htmlspecialchars($bulkCard['skipped_reasons'][$si]) ?></small><?php endif; ?></li><?php endforeach; ?><?php if ((int) ($bulkCard['skipped_more'] ?? 0) > 0): ?><li style="color:#8a9aa0">… ve <?= (int) $bulkCard['skipped_more'] ?> daha</li><?php endif; ?></ul></details><?php endif; ?><?php if (!empty($bulkCard['errors'])): ?><div style="margin-top:8px;color:#f3c4ba;font-size:12px">Hatalar: <?= htmlspecialchars(implode('; ', array_slice($bulkCard['errors'], 0, 5))) ?><?= count($bulkCard['errors']) > 5 ? ' …' : '' ?></div><?php endif; ?><?php if (($bulkCard['sub'] ?? '') === 'delete' && (int) ($bulkCard['done'] ?? 0) > 0): ?><div style="margin-top:8px"><a href="#trash" style="color:#d9f0b4;font-size:12px;font-weight:bold;text-decoration:underline">↩ Çöp kutusunda geri yükle →</a> <small style="color:#8a9aa0;font-size:11px">(silinen her özelliğin yanındaki ↩ butonuyla)</small></div><?php endif; ?><?php endif; ?></div><button type="button" onclick="dismissToast('bulkNotice','nexus_bulk_result')" class="nt-close" title="Kapat">✕</button></div><div class="nt-pill" onclick="toggleToast('bulkNotice')">⚡ Toplu işlem sonucu ▸</div></div><?php endif; ?>
<?php if ($actMsg !== ''): $actIcons = ['delete' => '🗑', 'restore' => '↩', 'move' => '↕']; $actSub = $actCard['sub'] ?? ''; ?><div id="actNotice" class="nexus-toast" data-ttl="8"><div class="nt-row"><div class="nt-body"><b><?= $actIcons[$actSub] ?? '✅' ?> İşlem sonucu:</b> <?= htmlspecialchars($actMsg) ?><?php if ($actSub === 'delete'): ?><div style="margin-top:10px;display:flex;align-items:center;gap:12px;flex-wrap:wrap"><form method="post" style="display:inline" onsubmit="return confirm('Özellik geri yüklensin mi? İlanlara aynı bölüm ve fiyat durumuyla tekrar eklenir.');"><input type="hidden" name="csrf" value="<?= htmlspecialchars($_SESSION['admin_csrf']) ?>"><input type="hidden" name="action" value="restore"><input type="hidden" name="id" value="<?= (int) ($actCard['id'] ?? 0) ?>"><button type="submit" style="background:#e6f8c7;color:#10211f;border:0;padding:5px 14px;font-size:12px;font-weight:bold;cursor:pointer;border-radius:5px">↩ Geri al</button></form><?php if ((int) ($actCard['affected'] ?? 0) > 0): ?><a href="#trash" style="color:#d9f0b4;font-size:12px;font-weight:bold;text-decoration:underline">Çöp kutusunda geri yükle →</a> <small style="color:#8a9aa0;font-size:11px">(özelliğin yanındaki ↩ butonuyla)</small><?php endif; ?></div><?php endif; ?></div><button type="button" onclick="dismissToast('actNotice','nexus_action_result')" class="nt-close" title="Kapat">✕</button></div><div class="nt-pill" onclick="toggleToast('actNotice')"><?= $actIcons[$actSub] ?? '✅' ?> İşlem sonucu ▸</div></div><?php endif; ?>
<?php if ($err): ?><p class="er"><?= htmlspecialchars($err) ?></p><?php endif; ?>
<?php if (!empty($pendingDelete)): ?>
<div class="c" style="border-color:#f3c4ba;background:#fff7f5"><h2>⚠ Silinecek: "<?= htmlspecialchars($pendingDelete['label']) ?>" <?= trash_ttl_badge((int) $ttlDays) ?></h2><?php if ($pendingDelete['affected']): ?><p>Bu özellik <b><?= count($pendingDelete['affected']) ?></b> kayıtlı ilanda kullanılıyor — <b><?= count(array_unique(array_column($pendingDelete['affected'], 'company_name'))) ?></b> tedarikçi etkilenir. Silerseniz aşağıdaki ilanlardan da kaldırılır (çöp kutusundan tek tıkla geri yüklenebilir):</p><p style="margin:10px 0 4px;font-weight:bold">📊 Tür kırılımı:</p><table style="border-collapse:collapse;margin:0 0 8px;font-size:13px;min-width:280px"><tr><th style="text-align:left;padding:7px 10px;border:1px solid #e1e5de;background:#f7f7f2">Tür</th><th style="text-align:right;padding:7px 10px;border:1px solid #e1e5de;background:#f7f7f2">İlan sayısı</th><th style="text-align:right;padding:7px 10px;border:1px solid #e1e5de;background:#f7f7f2">Pay</th></tr><?php $dtTot = (int) ($pendingDelete['by_type']['total'] ?? 0); foreach (['hotel' => 'Otel', 'villa' => 'Villa', 'yacht' => 'Yat'] as $tk => $tl): $dtN = (int) ($pendingDelete['by_type'][$tk] ?? 0); ?><tr><td style="padding:7px 10px;border:1px solid #e1e5de"><?= $tl ?></td><td style="padding:7px 10px;border:1px solid #e1e5de;text-align:right"><?= $dtN ?></td><td style="padding:7px 10px;border:1px solid #e1e5de;text-align:right"><?= $dtTot > 0 ? str_replace('.', ',', (string) round($dtN / $dtTot * 100, 1)) . '%' : '—' ?></td></tr><?php endforeach; ?><tr><td style="padding:7px 10px;border:1px solid #e1e5de;font-weight:bold;background:#f7f7f2">Toplam</td><td style="padding:7px 10px;border:1px solid #e1e5de;text-align:right;font-weight:bold;background:#f7f7f2"><?= (int) ($pendingDelete['by_type']['total'] ?? 0) ?></td><td style="padding:7px 10px;border:1px solid #e1e5de;text-align:right;font-weight:bold;background:#f7f7f2">100%</td></tr></table><?php if (!empty($pendingDelete['metrics'])): $pm = $pendingDelete['metrics']; ?><div style="display:flex;gap:10px;flex-wrap:wrap;margin:0 0 10px"><?php foreach (['media' => '🖼 Toplam görsel', 'rooms' => '🛏 Oda tipi', 'plans' => '💶 Fiyat planı', 'bookings' => '📅 Aktif rezervasyon'] as $pKey => $pLabel): ?><span style="display:inline-block;background:#f4f6f1;border:1px solid #e1e5de;border-radius:8px;padding:8px 14px;font-size:13px;cursor:pointer" onclick="toggleMetricDetail('<?= $pKey ?>')" title="İlan bazında görüntüle — hangi ilanda kaç tane"><?= $pLabel ?>: <b><?= (int) $pm[$pKey] ?></b> ▾</span><?php endforeach; ?></div><div id="metricDetailBox" style="display:none;margin:0 0 10px;padding:10px 12px;background:#f4f6f1;border:1px solid #d5dccf;border-radius:8px;font-size:13px;max-width:760px"></div><?php endif; ?><p style="margin:12px 0 4px;font-weight:bold">🏢 Etkilenen tedarikçiler (ilan bazında):</p><?php $bySupplier = []; foreach ($pendingDelete['affected'] as $a) { $bySupplier[(int) ($a['supplier_id'] ?? 0)][] = $a; } foreach ($bySupplier as $sid => $list): $company = (string) ($list[0]['company_name'] ?? ''); ?><div style="border:1px solid #f3c4ba;border-radius:8px;padding:10px 12px;margin:10px 0;background:#fff"><b>🏢 <?= htmlspecialchars($company) ?></b> <small style="color:#6b7774">— <a href="/nexustraveltech/admin/tedarikci-ilanlari?supplier_id=<?= (int) $sid ?>" style="color:#405b13;font-weight:bold;text-decoration:none" title="Bu tedarikçinin ilanlarını görüntüle"><?= count($list) ?> ilan →</a></small><?php if ($sid > 0): ?>&nbsp;<small style="color:#6b7774"><a href="/nexustraveltech/tedarikci/" target="_blank" rel="noopener" style="color:#6b7774">panel ↗</a></small><?php endif; ?><ul style="margin:8px 0 0"><?php foreach ($list as $a): ?><li><b><?= htmlspecialchars($a['name']) ?></b> <small style="color:#6b7774">(<?= $typeLabel($a['property_type']) ?>)</small><?php $secLabels = ['service_pricing' => 'fiyatlandırma', 'amenities' => 'olanaklar', 'activities' => 'aktiviteler', 'events' => 'etkinlikler']; if (!empty($a['sections'])): ?> <small style="color:#9d3b1c">· <?= count($a['sections']) ?> bölüm (<?= htmlspecialchars(implode(', ', array_map(fn($s) => $secLabels[$s] ?? $s, $a['sections']))) ?>)</small><?php endif; ?></li><?php endforeach; ?></ul></div><?php endforeach; ?><p style="color:#9d3b1c">Etkilenen tedarikçilerin panellerinde bu özellik artık görünmeyecek; yine de silme geri alınabilir — özellik çöp kutusuna taşınır, ilanlara aynı bölüm ve fiyat durumuyla geri yüklenebilir. Varsayılan geri yükleme penceresi: silinme + <?= (int) $ttlDays ?> gün; aşağıdan bu özellik için özel bir kalıcı silme tarihi seçebilirsiniz.</p><?php else: ?><p>Bu özellik şu an hiçbir ilanda kullanılmıyor — güvenle silinebilir.</p><?php endif; ?><form method="post" style="display:flex;gap:9px;margin-top:12px;flex-wrap:wrap;align-items:flex-end"><input type="hidden" name="csrf" value="<?= htmlspecialchars($_SESSION['admin_csrf']) ?>"><input type="hidden" name="action" value="delete"><input type="hidden" name="id" value="<?= (int) $pendingDelete['id'] ?>"><input type="hidden" name="confirmed" value="1"><label style="display:flex;flex-direction:column;gap:3px;font-size:12px;color:#405b13;font-weight:bold">Kalıcı silme tarihi (opsiyonel)<span style="display:inline-flex;align-items:center;gap:6px"><input type="date" name="purge_at" id="purgeAtSingle" min="<?= date('Y-m-d') ?>" value="<?= $prevPurge ?? '' ?>" style="padding:8px;border:1px solid #d8ded8;border-radius:5px;font-weight:normal" onchange="var b=document.getElementById('purgeBadge');if(this.value){b.style.display='inline-block';b.textContent='özel tarih: '+this.value}else{b.style.display='none'}"><?= trash_custom_date_badge($prevPurge ?? '', 'purgeBadge') ?></span><small style="color:#6b7774;font-weight:normal"><?php $prevPurge = substr((string)($pendingDelete['previous_purge_at'] ?? ''), 0, 10); ?>Boş bırakılırsa varsayılan TTL (silinme + <?= (int) $ttlDays ?> gün) geçerli olur.<?= $prevPurge ? '<br><small style="color:#8a6100;font-weight:normal">Önceki silmede <b>' . htmlspecialchars($prevPurge) . '</b> tarihi kullanılmıştı — kutuya girip aynı tarihi seçebilirsiniz.</small>' : '' ?></small></label><button style="background:#9d3b1c">Evet, sil ve ilanlardan kaldır</button><button type="button" onclick="this.form.purge_at.value='2026-08-18';if(confirm('Bugün kalıcı olarak silinecek. Geri alınamaz! Emin misiniz?'))this.form.submit();" style="background:#8e2410;border:2px solid #b0301a;margin-left:6px;padding:9px 14px;font-weight:bold;border-radius:5px;cursor:pointer;color:#fff;font-size:13px">⚡ Hemen kalıcı sil</button><button type="button" onclick="this.form.purge_at.value='<?= date("Y-m-d") ?>';if(confirm('Bug�n kal1c1 olarak silinecek. Geri al1namaz! Emin misiniz?'))this.form.submit();" style="background:#8e2410;border:2px solid #b0301a;margin-left:6px;padding:9px 14px;font-weight:bold;border-radius:5px;cursor:pointer;color:#fff;font-size:13px">âš Hemen kal1c1 sil</button><a href="/nexustraveltech/admin/ozellik-listeleri?export=delete_impact&feature_id=<?= (int) $pendingDelete['id'] ?>" style="display:inline-block;margin-top:10px;background:#f4f6f1;border:1px solid #d5dccf;color:#405b13;padding:8px 12px;font-size:13px;font-weight:bold;text-decoration:none;border-radius:5px">⬇ CSV indir (tedarikçi bazlı)</a><a href="/nexustraveltech/admin/ozellik-listeleri?export=delete_type&feature_id=<?= (int) $pendingDelete['id'] ?>" style="display:inline-block;margin-top:10px;background:#f4f6f1;border:1px solid #d5dccf;color:#405b13;padding:8px 12px;font-size:13px;font-weight:bold;text-decoration:none;border-radius:5px">⬇ Tür tablosu CSV</a><a href="/nexustraveltech/admin/ozellik-listeleri?export=delete_type&detail=1&feature_id=<?= (int) $pendingDelete['id'] ?>" style="display:inline-block;margin-top:10px;background:#eef3fb;border:1px solid #b9cbe8;color:#2b4a7a;padding:8px 12px;font-size:13px;font-weight:bold;text-decoration:none;border-radius:5px" title="Özet + etkilenen ilan adları (tür, tedarikçi, ID)">⬇ Tür tablosu CSV (ayrıntılı)</a><a href="/nexustraveltech/admin/ozellik-listeleri?view=supplier_summary&feature_id=<?= (int) $pendingDelete['id'] ?>" style="display:inline-block;margin-top:10px;background:#eef3fb;border:1px solid #b9cbe8;color:#2b4a7a;padding:8px 12px;font-size:13px;font-weight:bold;text-decoration:none;border-radius:5px">📊 Tedarikçi özeti</a></form><a href="/nexustraveltech/admin/ozellik-listeleri" style="display:inline-block;margin-top:10px;color:#6b7774;font-size:13px">Vazgeç</a></div>
<?php endif; ?>
<?php if (!empty($pendingBulk)): ?>
<div class="c" style="border-color:#f3c4ba;background:#fff7f5"><h2>⚠ <?= count($pendingBulk['items']) ?> özellik silinecek — toplam <?= (int) $pendingBulk['total_affected'] ?> ilan etkilenecek <span style="display:inline-block;background:#10211f;color:#f3c4ba;border:1px solid #9d3b1c;border-radius:12px;padding:2px 10px;font-size:12px;vertical-align:middle">🗑 Geri alınabilir silme</span></h2><?php if (!empty($pendingBulk['already_trash'])): $atr = $pendingBulk['already_trash']; ?><p style="margin:10px 0 0;color:#8a6100;background:#fff3cd;border:1px solid #e0c9a3;border-radius:6px;padding:8px 12px;font-size:13px">⏭ <b><?= count($atr) ?> seçili özellik zaten çöp kutusunda</b> — silme işleminde atlanacak (sayılmaz): <?= htmlspecialchars(implode(', ', array_map(fn($x) => (string) $x['label'], array_slice($atr, 0, 12)))) ?><?= count($atr) > 12 ? ', …' : '' ?>.</p><?php endif; ?><table style="border-collapse:collapse;margin:12px 0;font-size:13px;min-width:280px"><tr><th style="text-align:left;padding:7px 10px;border:1px solid #e1e5de;background:#f7f7f2">Tür</th><th style="text-align:right;padding:7px 10px;border:1px solid #e1e5de;background:#f7f7f2">İlan sayısı</th><th style="text-align:right;padding:7px 10px;border:1px solid #e1e5de;background:#f7f7f2">Pay</th></tr><?php $pbTot = (int) ($pendingBulk['total_affected'] ?? 0); foreach (['hotel' => 'Otel', 'villa' => 'Villa', 'yacht' => 'Yat'] as $tk => $tl): $pbN = (int) ($pendingBulk['by_type'][$tk] ?? 0); ?><tr><td style="padding:7px 10px;border:1px solid #e1e5de"><?= $tl ?></td><td style="padding:7px 10px;border:1px solid #e1e5de;text-align:right"><?= $pbN ?></td><td style="padding:7px 10px;border:1px solid #e1e5de;text-align:right"><?= $pbTot > 0 ? str_replace('.', ',', (string) round($pbN / $pbTot * 100, 1)) . '%' : '—' ?></td></tr><?php endforeach; ?><tr><td style="padding:7px 10px;border:1px solid #e1e5de;font-weight:bold;background:#f7f7f2">Toplam</td><td style="padding:7px 10px;border:1px solid #e1e5de;text-align:right;font-weight:bold;background:#f7f7f2"><?= (int) $pendingBulk['total_affected'] ?></td><td style="padding:7px 10px;border:1px solid #e1e5de;text-align:right;font-weight:bold;background:#f7f7f2">100%</td></tr></table><?php if (!empty($pendingBulk['metrics'])): $mk = $pendingBulk['metrics']; ?><div style="display:flex;gap:10px;flex-wrap:wrap;margin:10px 0 2px"><?php foreach (['media' => '🖼 Toplam görsel', 'rooms' => '🛏 Oda tipi', 'plans' => '💶 Fiyat planı', 'bookings' => '📅 Aktif rezervasyon'] as $mKey => $mLabel): ?><span style="display:inline-block;background:#f4f6f1;border:1px solid #e1e5de;border-radius:8px;padding:8px 14px;font-size:13px;cursor:pointer" onclick="toggleMetricDetail('<?= $mKey ?>')" title="İlan bazında görüntüle — hangi ilanda kaç tane"><?= $mLabel ?>: <b><?= (int) $mk[$mKey] ?></b> ▾</span><?php endforeach; ?></div><div id="metricDetailBox" style="display:none;margin:8px 0 4px;padding:10px 12px;background:#f4f6f1;border:1px solid #d5dccf;border-radius:8px;font-size:13px;max-width:760px"></div><?php endif; ?><?php $secNames = ['villa' => 'Villa', 'yacht' => 'Yat', 'amenity' => 'Otel olanakları', 'activity' => 'Otel aktiviteleri', 'event' => 'Otel etkinlikleri']; $itemsByCode = []; foreach ($pendingBulk['items'] as $bi) { $itemsByCode[$bi['code'] ?? 'other'][] = $bi; } ?><div style="margin:12px 0 8px"><p style="font-weight:bold;margin:0 0 6px;font-size:14px">📋 Silinecek özellikler (<?= count($pendingBulk['items']) ?>):</p><table style="border-collapse:collapse;width:100%;font-size:13px"><tr style="background:#f7f7f2"><th style="text-align:left;padding:7px 10px;border:1px solid #e1e5de">Özellik</th><th style="text-align:left;padding:7px 10px;border:1px solid #e1e5de">Bölüm / kategori</th><th style="text-align:right;padding:7px 10px;border:1px solid #e1e5de">Etkilenen ilan</th><th style="text-align:left;padding:7px 10px;border:1px solid #e1e5de">İlan adları</th></tr><?php foreach ($itemsByCode as $code => $group): $secLabel = $secNames[$code] ?? $code; ?><?php foreach ($group as $bi): ?><tr><td style="padding:7px 10px;border:1px solid #e1e5de;font-weight:bold"><?= htmlspecialchars($bi['label']) ?></td><td style="padding:7px 10px;border:1px solid #e1e5de"><span style="display:inline-block;background:#f4f6f1;border:1px solid #d5dccf;border-radius:10px;padding:1px 8px;font-size:11px"><?= htmlspecialchars($secLabel) ?></span></td><td style="padding:7px 10px;border:1px solid #e1e5de;text-align:right"><b><?= (int) $bi['affected_count'] ?></b></td><td style="padding:7px 10px;border:1px solid #e1e5de;font-size:12px;color:#6b7774"><?= $bi['affected_names'] ? htmlspecialchars(implode(', ', $bi['affected_names'])) . ($bi['affected_count'] > 5 ? ' <span style="color:#9d3b1c">… ve ' . ((int) $bi['affected_count'] - 5) . ' daha</span>' : '') : '<span style="color:#bcd98a">ilan yok</span>' ?></td></tr><?php endforeach; ?><?php endforeach; ?><tr style="background:#f4fbea"><td style="padding:7px 10px;border:1px solid #e1e5de;font-weight:bold" colspan="2">TOPLAM</td><td style="padding:7px 10px;border:1px solid #e1e5de;text-align:right;font-weight:bold"><?= count($pendingBulk['items']) ?> özellik · <?= (int) $pendingBulk['total_affected'] ?> ilan</td><td style="padding:7px 10px;border:1px solid #e1e5de"></td></tr></table></div><p style="color:#9d3b1c">🗑 Silinen özellikler <b>çöp kutusuna taşınır</b> ve <b>tek tıkla geri yüklenebilir</b> — kaldırıldığı ilanlara aynı bölüm ve fiyat durumuyla geri eklenir. Geri yükleme penceresi: silinme + <?= (int) $ttlDays ?> gün TTL (kontrol merkezinden ayarlanabilir). Aşağıdan tüm seçilenler için ortak bir kalıcı silme tarihi de belirleyebilirsiniz.</p><form method="post" style="margin-top:12px"><input type="hidden" name="csrf" value="<?= htmlspecialchars($_SESSION['admin_csrf']) ?>"><input type="hidden" name="action" value="bulk"><input type="hidden" name="sub" value="delete"><input type="hidden" name="confirm" value="1"><?php foreach ($pendingBulk['ids'] as $fid): ?><input type="hidden" name="ids[]" value="<?= (int) $fid ?>"><?php endforeach; ?><label style="display:flex;flex-direction:column;gap:3px;font-size:12px;color:#405b13;font-weight:bold">Kalıcı silme tarihi (opsiyonel — tüm seçilenler için ortak)<span style="display:inline-flex;align-items:center;gap:6px"><input type="date" name="purge_at" id="purgeAtBulk" min="<?= date('Y-m-d') ?>" value="<?= $prevPurge ?? '' ?>" style="padding:8px;border:1px solid #d8ded8;border-radius:5px;font-weight:normal" onchange="var b=document.getElementById('purgeBadgeBulk');if(this.value){b.style.display='inline-block';b.textContent='özel tarih: '+this.value}else{b.style.display='none'}"><?= trash_custom_date_badge($prevPurge ?? '', 'purgeBadgeBulk') ?></span><small style="color:#6b7774;font-weight:normal">Boş bırakılırsa her özellik kendi varsayılan TTL'siyle silinir.</small></label><button style="background:#9d3b1c">Evet, <?= count($pendingBulk['items']) ?> özelliği sil</button><a href="/nexustraveltech/admin/ozellik-listeleri?export=bulk_impact<?php foreach ($pendingBulk['ids'] as $fid): ?>&ids[]=<?= (int) $fid ?><?php endforeach; ?>" style="display:inline-block;margin-top:10px;background:#f4f6f1;border:1px solid #d5dccf;color:#405b13;padding:8px 12px;font-size:13px;font-weight:bold;text-decoration:none;border-radius:5px">⬇ CSV indir (tedarikçi bazlı)</a><a href="/nexustraveltech/admin/ozellik-listeleri?export=bulk_type<?php foreach ($pendingBulk['ids'] as $fid): ?>&ids[]=<?= (int) $fid ?><?php endforeach; ?>" style="display:inline-block;margin-top:10px;background:#f4f6f1;border:1px solid #d5dccf;color:#405b13;padding:8px 12px;font-size:13px;font-weight:bold;text-decoration:none;border-radius:5px">⬇ Tür tablosu CSV</a><a href="/nexustraveltech/admin/ozellik-listeleri?export=bulk_type&detail=1<?php foreach ($pendingBulk['ids'] as $fid): ?>&ids[]=<?= (int) $fid ?><?php endforeach; ?>" style="display:inline-block;margin-top:10px;background:#eef3fb;border:1px solid #b9cbe8;color:#2b4a7a;padding:8px 12px;font-size:13px;font-weight:bold;text-decoration:none;border-radius:5px" title="Özet + etkilenen ilan adları (tür, tedarikçi, ID)">⬇ Tür tablosu CSV (ayrıntılı)</a><a href="/nexustraveltech/admin/ozellik-listeleri?export=bulk_metrics<?php foreach ($pendingBulk['ids'] as $fid): ?>&ids[]=<?= (int) $fid ?><?php endforeach; ?><?php $mk = $pendingBulk['metrics'] ?? []; ?>&media=<?= (int) ($mk['media'] ?? 0) ?>&rooms=<?= (int) ($mk['rooms'] ?? 0) ?>&plans=<?= (int) ($mk['plans'] ?? 0) ?>&bookings=<?= (int) ($mk['bookings'] ?? 0) ?>" style="display:inline-block;margin-top:10px;background:#f4f6f1;border:1px solid #d5dccf;color:#405b13;padding:8px 12px;font-size:13px;font-weight:bold;text-decoration:none;border-radius:5px">⬇ Metrikler CSV</a><a href="/nexustraveltech/admin/ozellik-listeleri?export=bulk_metrics&detail=1<?php foreach ($pendingBulk['ids'] as $fid): ?>&ids[]=<?= (int) $fid ?><?php endforeach; ?><?php $mk = $pendingBulk['metrics'] ?? []; ?>&media=<?= (int) ($mk['media'] ?? 0) ?>&rooms=<?= (int) ($mk['rooms'] ?? 0) ?>&plans=<?= (int) ($mk['plans'] ?? 0) ?>&bookings=<?= (int) ($mk['bookings'] ?? 0) ?>" style="display:inline-block;margin-top:10px;background:#eef3fb;border:1px solid #b9cbe8;color:#2b4a7a;padding:8px 12px;font-size:13px;font-weight:bold;text-decoration:none;border-radius:5px" title="Özet + ilan başına satır (görsel/oda/plan/rezervasyon)">⬇ Metrikler CSV (ayrıntılı)</a><a href="/nexustraveltech/admin/ozellik-listeleri?view=supplier_summary<?php foreach ($pendingBulk['ids'] as $fid): ?>&ids[]=<?= (int) $fid ?><?php endforeach; ?>" style="display:inline-block;margin-top:10px;background:#eef3fb;border:1px solid #b9cbe8;color:#2b4a7a;padding:8px 12px;font-size:13px;font-weight:bold;text-decoration:none;border-radius:5px">📊 Tedarikçi özeti</a></form><a href="/nexustraveltech/admin/ozellik-listeleri" style="display:inline-block;margin-top:10px;color:#6b7774;font-size:13px">Vazgeç</a></div>
<?php endif; ?>
...
function expandToast(id){var el=document.getElementById(id);if(!el)return;el.classList.remove('collapsed')}
function toggleToast(id){var el=document.getElementById(id);if(!el)return;if(el.classList.contains('collapsed')){expandToast(id);clearTimeout(el.dataset.timer)}else{collapseToast(id)}}
function dismissToast(id,cookieName){var el=document.getElementById(id);if(el){el.dataset.gone='1';el.style.opacity='0';el.style.transform='translateX(30px)';setTimeout(function(){el.remove()},320)}document.cookie=cookieName+'=;expires=Thu, 01 Jan 1970 00:00:00 GMT;path=/';}
function initToastStack(){var ts=[].filter.call(document.querySelectorAll('.nexus-toast'),function(x){return x.parentNode});if(!ts.length)return;var stack=document.getElementById('toastStack');if(!stack){stack=document.createElement('div');stack.id='toastStack';stack.style.cssText='position:fixed;top:14px;right:14px;z-index:999;display:flex;flex-direction:column;align-items:stretch;gap:10px;width:min(420px,calc(100vw - 28px))';document.body.appendChild(stack)}ts.forEach(function(t){stack.appendChild(t);setupToast(t.id,parseInt(t.dataset.ttl||'8',10))})}
initToastStack();
var bulkFilter=null,catFilter=null,bulkBodyOrig={};
function toggleBulkFilter(st){bulkFilter=(bulkFilter===st)?null:st;applyBulkFilter()}
function toggleCatFilter(code){catFilter=(catFilter===code)?null:code;applyBulkFilter()}
function applyBulkFilter(){if(bulkFilter!==null&&!document.querySelector('.feat-check[data-state="'+bulkFilter+'"]:checked'))bulkFilter=null;if(catFilter!==null&&!document.querySelector('.feat-check[data-code="'+catFilter+'"]:checked'))catFilter=null;var active=bulkFilter!==null||catFilter!==null;document.querySelectorAll('[id^="catBody-"]').forEach(function(b){var code=b.id.replace('catBody-','');if(!active){if(bulkBodyOrig[code]!==undefined){b.style.display=bulkBodyOrig[code];delete bulkBodyOrig[code]};b.querySelectorAll('.feat-check').forEach(function(c){var ch=c.closest('.chip');if(ch)ch.style.display=''});return}if(bulkBodyOrig[code]===undefined)bulkBodyOrig[code]=b.style.display||'block';var shown=0;b.querySelectorAll('.feat-check').forEach(function(c){var ch=c.closest('.chip');var visible=c.checked&&(bulkFilter===null||c.dataset.state===bulkFilter)&&(catFilter===null||c.dataset.code===catFilter);if(ch)ch.style.display=visible?'':'none';if(visible)shown++});b.style.display=shown?'block':'none'});document.querySelectorAll('.bulk-badge').forEach(function(x){x.classList.toggle('filt',bulkFilter!==null&&x.dataset.b===bulkFilter)});document.querySelectorAll('.cat-badge').forEach(function(x){x.classList.toggle('filt',catFilter!==null&&x.dataset.cat===catFilter)})}
function bulkRefresh(){var sel=[].filter.call(document.querySelectorAll('.feat-check'),function(x){return x.checked});var c=sel.length;document.getElementById('bulkCount').textContent=c;var bar=document.getElementById('bulkBar');bar.style.display=c?'flex':'none';var on=sel.filter(function(x){return x.dataset.state==='on'}).length,off=c-on;var onBadge=bar.querySelector('[data-b="on"]'),offBadge=bar.querySelector('[data-b="off"]');onBadge.style.display=on?'inline-block':'none';onBadge.textContent='● '+on+' Aktif';offBadge.style.display=off?'inline-block':'none';offBadge.textContent='○ '+off+' Pasif';var cats=[['villa','Villa','#405b13'],['yacht','Yat','#1a3d6d'],['amenity','Otel olanakları','#8a6100'],['activity','Otel aktiviteleri','#7a4a8a'],['event','Otel etkinlikleri','#9d3b1c']],catBox=document.getElementById('bulkCatBadges'),catHtml='';cats.forEach(function(ca){var n=sel.filter(function(x){return x.dataset.code===ca[0]}).length,total=document.querySelectorAll('.feat-check[data-code="'+ca[0]+'"]').length,pct=total>0?Math.round(n/total*100):0;if(n>0)catHtml+='<span class="cat-badge" data-cat="'+ca[0]+'" style="display:inline-block;background:'+ca[2]+';color:#fff;border-radius:12px;padding:2px 10px;font-size:12px;font-weight:bold;cursor:pointer" onclick="toggleCatFilter(\''+ca[0]+'\')" title="'+ca[1]+' kataloğundan seçilen '+n+' / '+total+' özellik (%'+pct+') — yalnızca bunları görünür yap (tekrar tıkla: filtreyi kaldır)">'+ca[1]+': '+n+' (%'+pct+')</span>'});catBox.innerHTML=catHtml;document.querySelectorAll('.feat-check').forEach(function(x){var c=x.closest('.chip');if(c)c.classList.toggle('sel',x.checked)});var names=sel.map(function(x){return x.dataset.label||('#'+x.value)}),namesBox=document.getElementById('bulkNamesBox'),namesList=document.getElementById('bulkNamesList');if(namesBox){namesBox.style.display=c?'inline-block':'none';namesList.innerHTML='';if(c){names.slice(0,25).forEach(function(nm){var d=document.createElement('div');d.style.padding='2px 0';d.textContent='• '+nm;namesList.appendChild(d)});if(names.length>25){var m=document.createElement('div');m.style.cssText='color:#64716d;font-size:11px;padding-top:4px';m.textContent='… ve '+(names.length-25)+' daha';namesList.appendChild(m)};document.getElementById('bulkNamesSum').textContent='İsimler ('+names.length+') ▾'}}document.querySelectorAll('.selall').forEach(function(sa){var code=sa.dataset.code,all=document.querySelectorAll('.feat-check[data-code="'+code+'"]'),ch=document.querySelectorAll('.feat-check[data-code="'+code+'"]:checked');sa.checked=all.length>0&&ch.length===all.length});if(!c){bulkFilter=null;catFilter=null}applyBulkFilter()}
...
focusFeature();
function bulkGo(sub){var ids=[].map.call(document.querySelectorAll('.feat-check:checked'),function(x){return x.value});if(!ids.length)return;if(sub==='delete'&&!confirm(ids.length+' özellik silinecek. Önce etki özeti gösterilecek, devam edilsin mi?'))return;var f=document.getElementById('bulkForm');f.querySelectorAll('input[name="ids[]"],input[name^="expected_state["]').forEach(function(x){x.remove()});ids.forEach(function(v){var i=document.createElement('input');i.type='hidden';i.name='ids[]';i.value=v;f.appendChild(i);var ch=document.querySelector('.feat-check[value="'+v+'"]');if(ch&&ch.dataset.state){var e=document.createElement('input');e.type='hidden';e.name='expected_state['+v+']';e.value=ch.dataset.state;f.appendChild(e)}});document.getElementById('bulkSub').value=sub;f.submit()}
...
function toggleTrashPreview(id){var box=document.getElementById('tp-'+id);if(!box)return;if(box.style.display!=='none'){box.style.display='none';return}if(box.dataset.loaded){box.style.display='block';return}box.textContent='Yükleniyor…';box.style.display='block';fetch('/nexustraveltech/admin/ozellik-listeleri?qview=trash_listings&id='+id).then(function(r){return r.json()}).then(function(d){if(!d.ok){box.innerHTML='<b style="color:#9d3b1c">'+d.error+'</b>';return}if(!d.listings.length){box.innerHTML='<span style="color:#6b7774">Hiçbir ilandan kaldırılmamış.</span>';box.dataset.loaded='1';return}var html='<div style="font-weight:bold;margin-bottom:4px">“'+d.label+'” kaldırıldığı ilanlar ('+d.listings.length+'):</div>var sum=d.summary||{};if(sum.count>0){html+='<div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:6px;padding:6px 8px;background:#f4f6f1;border:1px solid #d5dccf;border-radius:6px;font-size:12px">';if(sum.total_price>0)html+='<span>\u{1F4B0} <b>'+sum.total_price.toLocaleString('tr-TR')+'</b> toplam fiyat</span>';var sd=sum.section_dist||{};var sdKeys=Object.keys(sd);if(sdKeys.length){html+='<span style="color:#6b7774">';sdKeys.forEach(function(k,i){if(i>0)html+=' \u00B7 ';html+=k+': <b>'+sd[k]+'</b>';});html+='</span>';}html+='</div>';}<div style="display:flex;align-items:center;gap:8px;margin:6px 0 4px;flex-wrap:wrap"><label style="font-size:12px;cursor:pointer;user-select:none"><input type="checkbox" class="tp-all" onchange="toggleTPAll(this,\"+id+\")"> T\u00fcm\u00fcn\u00fc se\u00e7</label><button class="mini" style="background:#405b13;color:#fff;font-weight:bold;border:0;padding:4px 10px;border-radius:5px;cursor:pointer;font-size:12px" onclick="restoreTPListings(\"+id+\")" title="Se\u00e7ili ilanlara \u00f6zelli\u011fi geri ekle">\u21A9 Se\u00e7ili ilanlara geri ekle</button></div><ul style="margin:0;padding-left:18px">d.listings.forEach(function(p){html+='<li><label style="display:inline-flex;align-items:center;gap:4px;cursor:pointer"><input type="checkbox" class="tp-check" data-tp=\"'+id+'\" value=\"'+p.id+'\" style="vertical-align:-2px"></label><a href="'+((p.property_type==='hotel')?'/nexustraveltech/tedarikci/otel-detay?product=':'/nexustraveltech/tedarikci/villa-detay?product=')+p.id+'" target="_blank" rel="noopener" style="color:#10211f;text-decoration:none;border-bottom:1px dashed #64716d;font-weight:bold">'+p.name+'</a> <small style="color:#6b7774">(#'+p.id+')</small>'+(p.sections.length?' <small style="color:#9d3b1c">\u00B7 '+p.sections.join(', ')+'</small>':'')+(p.price!==''?' <small style="color:#405b13">\u00B7 fiyat: '+p.price+'</small>':'')+'</li>'})html+='</ul>';box.innerHTML=html;box.dataset.loaded='1'}).catch(function(){box.innerHTML='<b style="color:#9d3b1c">Liste alınamadı (oturum süresi dolmuş olabilir).</b>'})}
function trashQuickSelect(filter){var checks=document.querySelectorAll('.trash-check');checks.forEach(function(x){x.checked=false});checks.forEach(function(x){var match=false;if(filter==='urgent')match=x.dataset.urgent==='1';else if(filter==='pending')match=x.dataset.pending==='1';else if(filter==='3days')match=parseInt(x.dataset.remain,10)<3;if(match)x.checked=true});trashRefresh();document.querySelectorAll('.trash-quick-filter').forEach(function(b){b.style.fontWeight=b.dataset.filter===filter?'bold':'normal';b.style.boxShadow=b.dataset.filter===filter?'0 0 0 2px '+b.style.borderColor:''})}
...
function toggleTPAll(ch,fid){document.querySelectorAll('.tp-check[data-tp="'+fid+'"]').forEach(function(x){x.checked=ch.checked})}
function restoreTPListings(fid){
var boxes=document.querySelectorAll('.tp-check[data-tp="'+fid+'"]');
var ids=[];
boxes.forEach(function(b){if(b.checked)ids.push(b.value)});
if(!ids.length){alert('Geri eklenecek ilan secin.');return}
if(!confirm(ids.length+' ilana ozellik geri eklenecek. Devam?'))return;
var fd=new FormData();ids.forEach(function(i){fd.append('listing_ids[]',i)});
fetch('/nexustraveltech/admin/ozellik-listeleri?qview=trash_restore_listings&id='+fid,{method:'POST',body:fd}).then(function(r){return r.json()}).then(function(d){
if(!d.ok){alert('Hata: '+(d.error||'Bilinmeyen hata'));return}
alert('\u2713 '+d.label+' '+d.restored+' ilana geri eklendi ('+d.sections+' bolum). Sayfa yenileniyor.');
location.reload()
}).catch(function(){alert('Istek basarisiz.')})
}
...
var BULK_METRIC_IDS=<?= json_encode(isset($pendingBulk['ids']) ? $pendingBulk['ids'] : (isset($pendingDelete['id']) ? [(int) $pendingDelete['id']] : []), JSON_UNESCAPED_UNICODE) ?>;
