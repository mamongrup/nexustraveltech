...
<?php endforeach; ?></div>
<?php if ($trash): ?><h2 id="trash" style="margin:26px 0 4px;border-top:1px solid #e2e6df;padding-top:20px">🗑 Çöp kutusu — geri alınabilir silmeler</h2><div class="c" style="border-color:#e0c9a3;background:#fdf9f2"><p style="color:#6b7774;font-size:13px;margin-top:0">Silinen özellikler burada durur ve tek tıkla geri yüklenebilir; kaldırıldığı ilanlara aynı bölüm ve fiyat durumuyla geri eklenir. Her özellik <b>silinme + <?= (int) $ttlDays ?> gün TTL</b> sonra kalıcı silinir (geri alınamaz); silme sırasında <b>özel kalıcı silme tarihi</b> verilenler o tarihte silinir.</p><?php foreach ($trash as $t): $delTs = strtotime((string) $t['deleted_at']) ?: time(); $customPurge = !empty($t['purge_at']); $purgeTs = $customPurge ? (strtotime((string) $t['purge_at']) ?: 0) : 0; if ($purgeTs <= 0) $purgeTs = $delTs + $ttlDays * 86400; $remainDays = (int) ceil(($purgeTs - time()) / 86400); $urgent = $remainDays <= 3; ?><span class="chip" style="opacity:1;border-color:#e0c9a3"><?= htmlspecialchars($t['label']) ?> <small style="color:#6b7774">(<?= htmlspecialchars($sectionTitles[$t['code']] ?? (string) $t['code']) ?> · <?= (int) $t['affected_count'] ?> ilan · silindi <?= htmlspecialchars(mb_substr((string) $t['deleted_at'], 0, 16)) ?> · <b style="color:<?= $urgent ? '#b0301a' : '#8a6100' ?>">kalıcı silme <?= date('Y-m-d', $purgeTs) ?> (<?= max(0, $remainDays) ?> gün)<?= $customPurge ? ' · özel tarih' : '' ?></b>)</small><form method="post" style="display:inline"><input type="hidden" name="csrf" value="<?= htmlspecialchars($_SESSION['admin_csrf']) ?>"><input type="hidden" name="action" value="restore"><input type="hidden" name="id" value="<?= (int) $t['id'] ?>"><button class="mini" style="background:#e6f8c7;color:#10211f;border:1px solid #bcd98a" title="Geri yükle">↩ Geri yükle</button></form></span><?php endforeach; ?></div><?php endif; ?>
</main><script>
