...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!hash_equals($_SESSION['admin_csrf'], (string) ($_POST['csrf'] ?? ''))) {
        ...
        if ($action === 'quick') {
            $date = (string) ($_POST['rate_date'] ?? date('Y-m-d'));
            $saved = 0;
            $up = db()->prepare("INSERT INTO fx_rates(base_currency,quote_currency,rate,rate_date,source) VALUES(?,?,?,?,'manual') ON CONFLICT(base_currency,quote_currency,rate_date) DO UPDATE SET rate=EXCLUDED.rate");
...
<section class="card"><h2 style="margin:0 0 6px;font-size:18px">Hızlı giriş — EUR / USD / GBP → TRY</h2><p style="color:#64716d;margin:0 0 12px;font-size:13px">TCMB dışında günlük kurları elle girmek için opsiyonel satırlar; her satır <b>TRY → ters kuru</b> da otomatik yazar (çapraz kur hesaplarında kullanılır). Boş bırakılan satır kaydedilmez.</p>
...
<div class="quick-row"><label>EUR → TRY</label><input name="quick_eur" type="number" step="0.000001" min="0.000001" placeholder="Örn. 38.50"></div>
<div class="quick-row"><label>USD → TRY</label><input name="quick_usd" type="number" step="0.000001" min="0.000001" placeholder="Örn. 33.20"></div>
<div class="quick-row"><label>GBP → TRY</label><input name="quick_gbp" type="number" step="0.000001" min="0.000001" placeholder="Örn. 42.80"></div>
