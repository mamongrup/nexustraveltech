...
echo "=== 1) TABLOLAR (" . count($requiredTables) . ") ===\n";
foreach ($requiredTables as $t) {
    echo (isset($found[$t]) ? '✓' : '✗') . ' ' . $t . "\n";
}
if ($missingTables) {
...
if ($dryRun && $pendingCount > 0) {
    echo "($pendingCount migration bekliyor — --dry-run uygulamadı)\n";
}
if ($failedMigs) {
    echo "Başarısız migration'lar: " . implode(', ', $failedMigs) . "\n";
}
...
if (strlen((string) ($config['app_encryption_key'] ?? '')) < 32) {
    echo "✗ app_encryption_key eksik veya 32 karakterden kısa\n";
...
echo "\n";
if ($errors) {
    echo 'SONUÇ: ' . count($errors) . ' sorun — ' . implode('; ', array_slice($errors, 0, 10)) . "\n";
    exit(1);
}
echo 'SONUÇ: Tüm kontroller başarılı — ' . count($requiredTables) . " tablo, kritik kolonlar ve " . count($migrationFiles) . " migration hazır.\n";
