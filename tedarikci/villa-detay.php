...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['room_action'] ?? '') !== 'duplicate' && ($_POST['action'] ?? '') !== 'publish') {
  if (!csrf_valid($_POST['csrf_token'] ?? null)) { $error = 'Güvenlik doğrulaması yenilendi. Lütfen tekrar deneyin.'; }
    ...
    $amenityList = property_feature_lists($isYacht ? 'yacht' : 'villa');
    $servicePricing = [];
...
<?php $readiness = listing_readiness($listing); $linkFor = ['rooms' => '/nexustraveltech/tedarikci/villa-detay?product='.$id.'#sec-04', 'rates' => '/nexustraveltech/tedarikci/fiyat-kontenjan?property='.$id, 'inventory' => '/nexustraveltech/tedarikci/fiyat-kontenjan?property='.$id, 'media' => '/nexustraveltech/tedarikci/villa-detay?product='.$id.'#sec-05', 'description' => '/nexustraveltech/tedarikci/villa-detay?product='.$id.'#sec-02', 'location' => '/nexustraveltech/tedarikci/villa-detay?product='.$id.'#sec-01', 'pool' => '/nexustraveltech/tedarikci/villa-detay?product='.$id.'#sec-01', 'home_port' => '/nexustraveltech/tedarikci/villa-detay?product='.$id.'#sec-01', 'crew' => '/nexustraveltech/tedarikci/villa-detay?product='.$id.'#sec-01', 'ical' => '/nexustraveltech/tedarikci/ical-takvimler?property='.$id.'#sync', 'rules' => '/nexustraveltech/tedarikci/satis-kurallari?property='.$id]; /* Bölümler tek kaynaktan numaralanır — yeni bölüm eklerken yalnızca bu diziye satır ekle. */ $editorSections = ['Kimlik, sınıflandırma & konum' => 'Kimlik & konum', 'Satış içeriği & operasyon' => 'Satış içeriği', 'Özellikler & hizmetler' => 'Özellikler', $unitLabel.' envanteri & fiyat' => 'Birim & fiyat', 'Görseller' => 'Görseller', 'Acente komisyonu & tahsilat' => 'Komisyon & tahsilat', 'İptal & iade şartları' => 'İptal & iade']; $editorToc = []; $editorN = 0; foreach ($editorSections as $fullTitle => $shortTitle) { $editorN++; $editorToc[] = ['id' => 'sec-'.str_pad((string)$editorN, 2, '0', STR_PAD_LEFT), 'no' => str_pad((string)$editorN, 2, '0', STR_PAD_LEFT), 'title' => $fullTitle, 'short' => $shortTitle]; } $editorN = 0; ?>
<section class="publish-panel"><div class="publish-head"><div><span>YAYINA ALMA AKIŞI</span><h3>Hazırlık kontrol listesi · <b><?= $readiness['score'] ?>/100</b></h3><p>Tüm çekirdek kalemler (villa: havuz, yat: liman + mürettebat, ikisi için de aktif iCal bağlantısı dahil) tamamlandığında ürün tek tıkla yayına alınır; satış kuralı opsiyoneldir.</p></div><?php if ($listing['status'] === 'active'): ?><b class="status active-status">YAYINDA</b><?php else: ?><form method="post" class="publish-form"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars(csrf_token()) ?>"><input type="hidden" name="action" value="publish"><?php if ($readiness['ready']): ?><button class="primary-button" onclick="return confirm('Ürün yayına alınsın mı? Acente müsaitlik sorgularında görünür olur.');">Yayına al →</button><?php else: ?><span class="ghost-button disabled" title="Eksik kalemler tamamlanmadan yayına alınamaz.">Yayına al</span><?php endif; ?></form><?php endif; ?></div><ul class="readiness-check"><?php foreach ($readiness['items'] as $item): $warn = !empty($item['warn']); $cls = !$item['ok'] ? 'missing' : ($warn ? 'warn' : 'ok'); $icon = !$item['ok'] ? '✗' : ($warn ? '⚠' : '✓'); ?><li class="<?= $cls ?>"><?= $icon ?> <?= htmlspecialchars($item['label']) ?><small><?= htmlspecialchars($item['detail']) ?></small><?php if (!$item['ok']): ?><a href="<?= htmlspecialchars($linkFor[$item['key']] ?? '#') ?>">Doldur →</a><?php elseif ($warn): ?><a href="<?= htmlspecialchars($linkFor[$item['key']] ?? '#') ?>">İncele →</a><?php else: ?><a class="readiness-all-view" href="<?= htmlspecialchars($linkFor[$item['key']] ?? '#') ?>">Gör →</a> <button type="button" class="ical-copy-btn" data-copy="<?= htmlspecialchars($linkFor[$item['key']] ?? '#') ?>" title="URL'yi kopyala">⧉ Kopyala</button><?php endif; ?><?php $icalUrlVisible = empty($item['urls']) ? false : (!(bool) platform_setting('ical_url_published_only', false) || $listing['status'] === 'active'); if ($icalUrlVisible): ?><span class="ical-copy-list"><?php foreach ($item['urls'] as $icalUrl): $icalQ = (string) parse_url($icalUrl, PHP_URL_QUERY); $icalShort = 'api/ical?' . mb_substr($icalQ, 0, 10) . '…' . mb_substr($icalQ, 30, 4) . '…'; ?><span class="ical-copy-row"><code title="<?= htmlspecialchars($icalUrl) ?>"><?= htmlspecialchars($icalShort) ?></code><button type="button" class="ical-copy-btn" data-copy="<?= htmlspecialchars($icalUrl) ?>">Kopyala</button></span><?php endforeach; ?></span><?php endif; ?></li><?php endforeach; ?></ul></section>
...
<div class="editor-save"><a href="/nexustraveltech/tedarikci/tesisler">Daha sonra tamamla</a><button type="submit">Tüm detayları kaydet →</button></div></form><script>document.querySelectorAll('.editor-toc nav a').forEach(function(a){a.addEventListener('click',function(e){e.preventDefault();var id=this.getAttribute('href').slice(1),el=document.getElementById(id);if(el){var y=el.getBoundingClientRect().top+window.scrollY-110;window.scrollTo({top:y,behavior:'smooth'})}})});var tocLinks=document.querySelectorAll('.editor-toc nav a'),sections=document.querySelectorAll('.editor-section');function markToc(){var pos=window.scrollY+140,current='';sections.forEach(function(s){if(s.offsetTop<=pos)current=s.id});tocLinks.forEach(function(a){a.classList.toggle('active',a.getAttribute('href')==='#'+current)})}window.addEventListener('scroll',markToc,{passive:true});markToc();if(location.hash){var hEl=document.querySelector(location.hash);if(hEl){setTimeout(function(){var hy=hEl.getBoundingClientRect().top+window.scrollY-110;window.scrollTo({top:hy,behavior:'smooth'})},80)}}function fallbackCopy(text){var ta=document.createElement('textarea');ta.value=text;ta.style.position='fixed';ta.style.opacity='0';document.body.appendChild(ta);ta.select();try{document.execCommand('copy')}catch(e){}document.body.removeChild(ta)}document.querySelectorAll('.ical-copy-btn').forEach(function(b){b.addEventListener('click',function(){var url=this.getAttribute('data-copy'),btn=this;function done(){btn.textContent='Kopyalandı ✓';btn.classList.add('copied');setTimeout(function(){btn.textContent='Kopyala';btn.classList.remove('copied')},1600)}if(navigator.clipboard&&navigator.clipboard.writeText){navigator.clipboard.writeText(url).then(done).catch(function(){fallbackCopy(url);done()})}else{fallbackCopy(url);done()}})});</script></div><?php supply_end(); ?>
