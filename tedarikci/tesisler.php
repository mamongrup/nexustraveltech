...
<article class="property-card">
    ...
    <div class="readiness-block">
        <div class="readiness-head"><span>YAYIN HAZIRLIĞI</span><b class="readiness-score-badge" data-readiness-modal="readiness-modal-<?= (int) $item['id'] ?>" style="cursor:pointer" title="Karşı karşıya özetini aç"><?= $readiness['score'] ?>/100</b></div>
        <?php if ($readiness['missing_count'] > 0): ?><p class="readiness-scorecard"><?= score_done_text((int) $readiness['ok_count']) ?> — <?= score_remain_text((int) $readiness['missing_count']) ?>.</p><?php endif; ?>
<?php $__miss = array_filter($readiness['items'], fn($i) => !$i['ok']); if ($__miss): $__first = reset($__miss); $__badId = 0; $__connId = 0; foreach ($readiness['items'] as $__ci) { if ($__ci['key'] === 'channel' && !empty($__ci['first_bad_id'])) { $__badId = (int) $__ci['first_bad_id']; } if ($__ci['key'] === 'ical' && !empty($__ci['first_conn_id'])) { $__connId = (int) $__ci['first_conn_id']; } } if (!isset($editorToc)) { $__jumpEditorSections = ['Kimlik, sınıflandırma & konum' => 'Kimlik & konum', 'Satış içeriği & operasyon' => 'Satış içeriği', 'Olanaklar, aktiviteler & etkinlikler' => 'Olanaklar', 'Oda envanteri & fiyat' => 'Oda envanteri', 'Görseller' => 'Görseller', 'Acente komisyonu & tahsilat' => 'Komisyon & tahsilat', 'İptal & iade şartları' => 'İptal & iade']; $__jumpToc = derive_editor_toc($__jumpEditorSections); $editorToc = $__jumpToc['toc']; $editorSections = $__jumpEditorSections; } $__jumpKeyMap = ['rooms' => 'Oda envanteri', 'media' => 'Görseller', 'description' => 'Satış içeriği', 'location' => 'Kimlik & konum', 'pool' => 'Kimlik & konum', 'home_port' => 'Kimlik & konum', 'crew' => 'Kimlik & konum']; $__jumpMeta = derive_sec_metadata($editorToc, $editorSections, $__jumpKeyMap); $__jumpSec = $__jumpMeta['secAnchor']; $__jumpLink = ($item['property_type'] === 'hotel' ? '/nexustraveltech/tedarikci/otel-detay?product=' : '/nexustraveltech/tedarikci/villa-detay?product=') . (int) $item['id'] . ($__jumpSec[$__first['key']] ?? ''); if (in_array($__first['key'], ['rates', 'inventory'], true)) { $__jumpLink = '/nexustraveltech/tedarikci/fiyat-kontenjan?property=' . (int) $item['id']; } elseif ($__first['key'] === 'ical') { $__jumpLink = '/nexustraveltech/tedarikci/ical-takvimler?property=' . (int) $item['id'] . ($__connId > 0 ? '#sync-' . $__connId : '#sync'); } elseif ($__first['key'] === 'channel') { $__jumpLink = '/nexustraveltech/tedarikci/dagitim-merkezi' . ($__badId > 0 ? '#channel-retry-' . $__badId : ''); } elseif ($__first['key'] === 'rules') { $__jumpLink = '/nexustraveltech/tedarikci/satis-kurallari?property=' . (int) $item['id']; } $__jumpLabel = listing_missing_jump_label($__first['key']); ?>
        ...
        <div id="readiness-all-<?= (int) $item['id'] ?>" class="readiness-all"<?= $autoOpenAll ? '' : ' hidden' ?>>
            ...
            <div class="readiness-all-row <?= $riCls ?>"><span class="readiness-all-icon"><?= $riIcon ?></span><div class="readiness-all-text"><b><?= htmlspecialchars($ri['label']) ?><?php if ($ri['key'] === 'rules' && !$ri['ok']): ?> <em>(<?= readiness_labels()['optional'] ?>)</em><?php endif; ?></b><small><?= htmlspecialchars($ri['detail']) ?></small><?php if (!$ri['ok'] && $ri['key'] !== 'rules'): ?> <em class="readiness-gain" title="<?= readiness_gain_tooltip() ?>">+<?= (int) ($ri['weight'] ?? 0) ?></em><?php endif; ?></div><?php if (!$ri['ok']): ?><a title="<?= htmlspecialchars($linkTooltips[$ri['key']] ?? '') ?>" href="<?= htmlspecialchars($missLinks[$ri['key']] ?? $editBase) ?>"><?= readiness_labels()['fill'] ?> →</a> <span class="ical-copy-row"><code title="<?= htmlspecialchars($missLinks[$ri['key']] ?? $editBase) ?>"><?= htmlspecialchars($copyShort((string) ($missLinks[$ri['key']] ?? $editBase))) ?></code><button type="button" class="ical-copy-btn" data-copy="<?= htmlspecialchars($missLinks[$ri['key']] ?? $editBase) ?>"><?= readiness_labels()['copy'] ?></button></span><?php elseif ($riWarn): ?><a title="<?= htmlspecialchars($linkTooltips[$ri['key']] ?? '') ?>" href="<?= htmlspecialchars($warnLinks[$ri['key']] ?? '/nexustraveltech/tedarikci/ical-takvimler') ?>"><?= readiness_labels()['inspect'] ?> →</a> <a class="readiness-all-view" title="<?= htmlspecialchars($linkTooltips[$ri['key']] ?? '') ?>" href="<?= htmlspecialchars($missLinks[$ri['key']] ?? $editBase) ?>"><?= readiness_labels()['view'] ?> →</a> <?php if ($riWarn && isset($secTitles[$ri["key"]], $secAnchor[$ri["key"]])): ?><em class="readiness-sec-hint"><?= htmlspecialchars($secTitles[$ri["key"]] . " " . $secAnchor[$ri["key"]]) ?></em><?php endif; ?> <span class="ical-copy-row"><code title="<?= htmlspecialchars($warnLinks[$ri['key']] ?? '/nexustraveltech/tedarikci/ical-takvimler') ?>"><?= htmlspecialchars($copyShort((string) ($warnLinks[$ri['key']] ?? '/nexustraveltech/tedarikci/ical-takvimler'))) ?></code><button type="button" class="ical-copy-btn" data-copy="<?= htmlspecialchars($warnLinks[$ri['key']] ?? '/nexustraveltech/tedarikci/ical-takvimler') ?>"><?= readiness_labels()['copy'] ?></button></span><?php else: ?><a class="readiness-all-view" title="<?= htmlspecialchars($linkTooltips[$ri['key']] ?? '') ?>" href="<?= htmlspecialchars($missLinks[$ri['key']] ?? $editBase) ?>"><?= readiness_labels()['view'] ?> →</a> <span class="ical-copy-row"><code title="<?= htmlspecialchars($missLinks[$ri['key']] ?? $editBase) ?>"><?= htmlspecialchars($copyShort((string) ($missLinks[$ri['key']] ?? $editBase))) ?></code><button type="button" class="ical-copy-btn" data-copy="<?= htmlspecialchars($missLinks[$ri['key']] ?? $editBase) ?>"><?= readiness_labels()['copy'] ?></button></span><?php if (!$ri['ok'] && isset($secTitles[$ri['key']], $secAnchor[$ri['key']])): ?> <em class="readiness-sec-hint"><?= htmlspecialchars($secTitles[$ri['key']] . ' ' . $secAnchor[$ri['key']]) ?></em><?php endif; ?><?php endif; ?><?php if ($ri['key'] === 'media' && $ri['ok']): $mp = $pdo->prepare("SELECT file_path, is_cover, title FROM property_media WHERE property_id=? ORDER BY is_cover DESC, sort_order, id LIMIT 8"); $mp->execute([(int) $item['id']]); $mediaPrev = $mp->fetchAll(); if ($mediaPrev): ?><span class="readiness-media-preview" style="display:flex;flex-wrap:wrap;gap:6px;margin-top:8px;align-items:center"><?php foreach ($mediaPrev as $mediaIdx => $mv): $src = '/nexustraveltech/' . ltrim((string) $mv['file_path'], '/'); ?><span style="position:relative;display:inline-block"><?php if ($mv['is_cover']): ?><span style="position:absolute;left:0;bottom:0;background:#b26a00;color:#fff;font-size:8px;font-weight:700;padding:1px 4px;border-radius:0 4px 0 4px;letter-spacing:.04em;z-index:1"><?= media_badge_cover() ?></span><?php endif; ?><img src="<?= htmlspecialchars($src) ?>" alt="" loading="lazy" data-desc="<?= htmlspecialchars((string)($mv['title'] ?? '')) ?>" style="width:56px;height:56px;object-fit:cover;border-radius:6px;border:1px solid #e1e5de" title="<?= ($mediaIdx+1)?>/<?= count($mediaPrev) ?><?= $mv['is_cover'] ? ' · Kapak' : '' ?>"<?= $mv['is_cover'] ? ' style="width:56px;height:56px;object-fit:cover;border-radius:6px;border:2px solid #b26a00"' : '' ?>></span><?php endforeach; ?></span><a class="readiness-media-more" href="<?= htmlspecialchars($missLinks['media'] ?? $editBase) ?>" style="font-size:11px;color:#4a6d8c;text-decoration:none;border-bottom:1px dotted #4a6d8c"><?= readline_all() ?> →</a></span><?php else: ?><a class="readiness-media-empty" href="<?= htmlspecialchars(($editBase ?? '#sec-05') . '#sec-05') ?>" style="display:block;margin-top:6px;font-size:12px;color:#4a6d8c;text-decoration:underline;border-bottom:1px dotted #4a6d8c">📷 <?= section_name('Görseller') ?> — <?= section_name('Görseller') ?> →</a><?php endif; endif; ?></div>
            <?php endforeach; ?>
        </div>
    </div>
    <div id="readiness-modal-<?= (int) $item['id'] ?>" class="readiness-modal-backdrop" hidden>
        <div class="readiness-modal">
            <div class="readiness-modal-head">
                <span><b><?= htmlspecialchars($item['name']) ?></b> · YAYIN HAZIRLIĞI</span>
                <b class="readiness-score-badge-modal" style="font-size:22px;<?= $readiness['score'] >= 100 ? 'color:#2e7d32' : ($readiness['score'] >= 70 ? 'color:#b26a00' : 'color:#b0301a') ?>"><?= $readiness['score'] ?>/100</b>
                <button type="button" class="readiness-modal-close" title="Kapat">✕</button>
            </div>
            <div class="readiness-modal-bar"><i style="width:<?= (int) $readiness['score'] ?>%"></i></div>
            <?php if ($readiness['missing_count'] > 0): ?><p class="readiness-scorecard" style="margin:4px 0 8px"><?= score_done_text((int) $readiness['ok_count']) ?> — <?= score_remain_text((int) $readiness['missing_count']) ?>.</p><?php endif; ?>
            <div class="readiness-modal-items">
            <?php foreach ($readiness['items'] as $mi): $mCls = !$mi['ok'] ? 'missing' : (!empty($mi['warn']) ? 'warn' : 'ok'); $mIcon = !$mi['ok'] ? '✗' : (!empty($mi['warn']) ? '⚠' : '✓'); ?>
            <div class="readiness-all-row <?= $mCls ?>"><span class="readiness-all-icon"><?= $mIcon ?></span><div class="readiness-all-text"><b><?= htmlspecialchars($mi['label']) ?><?php if ($mi['key'] === 'rules' && !$mi['ok']): ?> <em>(<?= readiness_labels()['optional'] ?>)</em><?php endif; ?></b><small><?= htmlspecialchars($mi['detail']) ?></small></div></div>
            <?php endforeach; ?>
            </div>
            <div class="readiness-modal-actions">
                <?php if ($readiness['score'] >= 100): ?><a class="readiness-publish-btn" href="#" style="background:#2e7d32;color:#fff;padding:6px 16px;border-radius:6px;text-decoration:none;font-weight:600">Yayına al ✓</a><?php endif; ?>
                <a href="<?= htmlspecialchars($missLinks[$__first['key']] ?? $editBase) ?>" style="padding:6px 16px;border-radius:6px;border:1px solid #d8ded8;text-decoration:none;color:#4a6d8c">İlk eksik bolum →</a>
                <?php $modalWarns = array_filter($readiness['items'], fn($i) => !empty($i['warn'])); $modalFirstWarn = $modalWarns ? reset($modalWarns) : null; if ($modalFirstWarn): ?>
                <a href="<?= htmlspecialchars($warnLinks[$modalFirstWarn['key']] ?? $editBase) ?>" style="padding:6px 16px;border-radius:6px;border:1px solid #f0c4bc;background:#fff8f6;text-decoration:none;color:#b0301a;font-weight:600">⚠ <?= htmlspecialchars($modalFirstWarn['label']) ?> →</a>
                <?php endif; ?>
            </div>
        </div>
    </div>
    <div class="property-actions">
        ...
        <?php else: ?>
        <span class="ghost-button disabled" title="<?= readline_publish_disabled_tip() ?>."><?= readline_publish() ?></span>
        <?php endif; ?>
    ...
    <div id="ical-card-<?= (int) $item['id'] ?>" class="ical-card-box" hidden>
        ...
        <span class="ical-copy-row"><code title="<?= htmlspecialchars($icalUrl) ?>"><?= htmlspecialchars($icalShort) ?></code><button type="button" class="ical-copy-btn" data-copy="<?= htmlspecialchars($icalUrl) ?>"><?= readiness_labels()['copy'] ?></button></span>
        <?php endforeach; ?>
        ...
        <div id="ical-help-<?= (int) $item['id'] ?>" class="ical-help-box" hidden>
            <ol>
                <li>Yukarıdaki <b><?= readiness_labels()['copy'] ?></b> butonuyla bu ilanın iCal URL'sini panoya alın.</li>
