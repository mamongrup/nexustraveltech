...
<article class="property-card">
    ...
    <div class="readiness-block">
        ...
        <?php $missing = array_filter($readiness['items'], fn($i) => !$i['ok']); $warnings = array_filter($readiness['items'], fn($i) => !empty($i['warn'])); $warnLinks = ['ical' => '/nexustraveltech/tedarikci/ical-takvimler?property=' . (int) $item['id'] . '#sync']; $editBase = $item['property_type'] === 'hotel' ? '/nexustraveltech/tedarikci/otel-detay?product=' . (int) $item['id'] : '/nexustraveltech/tedarikci/villa-detay?product=' . (int) $item['id']; $secAnchor = ['rooms' => '#sec-04', 'media' => '#sec-05', 'description' => '#sec-02', 'location' => '#sec-01', 'pool' => '#sec-01', 'home_port' => '#sec-01', 'crew' => '#sec-01']; $secTitles = ['rooms' => ($item['property_type'] === 'hotel' ? 'Oda envanteri' : 'Birim & fiyat'), 'media' => 'Görseller', 'description' => 'Satış içeriği', 'location' => 'Kimlik & konum', 'pool' => 'Kimlik & konum', 'home_port' => 'Kimlik & konum', 'crew' => 'Kimlik & konum']; $linkTooltips = ['rooms' => 'Oda / birim envanteri', 'rates' => 'Fiyat & kontenjan', 'inventory' => 'Fiyat & kontenjan', 'media' => 'Görseller', 'description' => 'Satış içeriği', 'location' => 'Kimlik & konum', 'pool' => 'Kimlik & konum', 'home_port' => 'Kimlik & konum', 'crew' => 'Kimlik & konum', 'ical' => 'iCal takvimleri', 'rules' => 'Satış kuralları']; $missLinks = ['rooms' => $editBase . $secAnchor['rooms'], 'rates' => '/nexustraveltech/tedarikci/fiyat-kontenjan?property=' . (int) $item['id'], 'inventory' => '/nexustraveltech/tedarikci/fiyat-kontenjan?property=' . (int) $item['id'], 'media' => $editBase . $secAnchor['media'], 'description' => $editBase . $secAnchor['description'], 'location' => $editBase . $secAnchor['location'], 'pool' => $editBase . $secAnchor['pool'], 'home_port' => $editBase . $secAnchor['home_port'], 'crew' => $editBase . $secAnchor['crew'], 'ical' => '/nexustraveltech/tedarikci/ical-takvimler', 'rules' => '/nexustraveltech/tedarikci/satis-kurallari?property=' . (int) $item['id']]; ?>
        <?php if ($warnings): ?>
        ...
        <?php endif; ?>
        <?php if ($missing): ?>
        <ul class="readiness-missing"><?php foreach ($missing as $m): ?><li><a class="readiness-miss-link" title="<?= htmlspecialchars($linkTooltips[$m['key']] ?? '') ?>" href="<?= htmlspecialchars($missLinks[$m['key']] ?? $editBase) ?>">✗ <?= htmlspecialchars($m['label']) ?><?php if ($m['key'] === 'rules'): ?> <em>(opsiyonel)</em><?php endif; ?><?php if (isset($secTitles[$m['key']], $secAnchor[$m['key']])): ?> <em class="readiness-sec-hint"><?= htmlspecialchars($secTitles[$m['key']] . ' ' . $secAnchor[$m['key']]) ?></em><?php endif; ?> →</a></li><?php endforeach; ?></ul>
        <?php elseif (!$warnings): ?>
        ...
        <div id="readiness-all-<?= (int) $item['id'] ?>" class="readiness-all"<?= $autoOpenAll ? '' : ' hidden' ?>>
            ...
            <div class="readiness-all-row <?= $riCls ?>"><span class="readiness-all-icon"><?= $riIcon ?></span><div class="readiness-all-text"><b><?= htmlspecialchars($ri['label']) ?><?php if ($ri['key'] === 'rules' && !$ri['ok']): ?> <em>(opsiyonel)</em><?php endif; ?></b><small><?= htmlspecialchars($ri['detail']) ?></small></div><?php if (!$ri['ok']): ?><a title="<?= htmlspecialchars($linkTooltips[$ri['key']] ?? '') ?>" href="<?= htmlspecialchars($missLinks[$ri['key']] ?? $editBase) ?>">Doldur →</a><?php elseif ($riWarn): ?><a title="<?= htmlspecialchars($linkTooltips[$ri['key']] ?? '') ?>" href="<?= htmlspecialchars($warnLinks[$ri['key']] ?? '/nexustraveltech/tedarikci/ical-takvimler') ?>">İncele →</a><?php else: ?><a class="readiness-all-view" title="<?= htmlspecialchars($linkTooltips[$ri['key']] ?? '') ?>" href="<?= htmlspecialchars($missLinks[$ri['key']] ?? $editBase) ?>">Gör →</a> <button type="button" class="ical-copy-btn" data-copy="<?= htmlspecialchars($missLinks[$ri['key']] ?? $editBase) ?>" title="URL'yi kopyala">⧉ Kopyala</button><?php endif; ?><?php if ($ri['key'] === 'media' && $ri['ok']): $mp = $pdo->prepare("SELECT file_path, is_cover FROM property_media WHERE property_id=? ORDER BY is_cover DESC, sort_order, id LIMIT 8"); $mp->execute([(int) $item['id']]); $mediaPrev = $mp->fetchAll(); if ($mediaPrev): ?><button type="button" class="readiness-media-toggle" data-target="media-preview-<?= (int) $item['id'] ?>" data-open-label="<?= count($mediaPrev) ?> görseli göster" style="font-size:11px;color:#405b13;background:none;border:1px dashed #bcd98a;border-radius:12px;padding:2px 10px;cursor:pointer"><?= count($mediaPrev) ?> görseli göster</button><span class="readiness-media-preview" id="media-preview-<?= (int) $item['id'] ?>" hidden style="display:flex;flex-wrap:wrap;gap:6px;margin-top:8px"><?php foreach ($mediaPrev as $mv): $src = '/nexustraveltech/' . ltrim((string) $mv['file_path'], '/'); ?><img src="<?= htmlspecialchars($src) ?>" alt="" loading="lazy" style="width:56px;height:56px;object-fit:cover;border-radius:6px;border:1px solid #e1e5de"<?= $mv['is_cover'] ? ' title="Kapak" style="width:56px;height:56px;object-fit:cover;border-radius:6px;border:2px solid #b26a00"' : '' ?>><?php endforeach; ?></span><?php endif; endif; ?></div>
            <?php endforeach; ?>
        </div>
    </div>
