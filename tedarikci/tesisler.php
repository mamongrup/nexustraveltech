...
<?php $__miss = array_filter($readiness['items'], fn($i) => !$i['ok']); if ($__miss): $__first = reset($__miss); ?>
        ...
        <div id="readiness-all-<?= (int) $item['id'] ?>" class="readiness-all"<?= $autoOpenAll ? '' : ' hidden' ?>>
            ...
            <div class="readiness-all-row <?= $riCls ?>"><span class="readiness-all-icon"><?= $riIcon ?></span><div class="readiness-all-text"><b><?= htmlspecialchars($ri['label']) ?><?php if ($ri['key'] === 'rules' && !$ri['ok']): ?> <em>(opsiyonel)</em><?php endif; ?></b><small><?= htmlspecialchars($ri['detail']) ?></small><?php if (!$ri['ok'] && $ri['key'] !== 'rules'): ?> <em class="readiness-gain" title="Bu kalem tamamlanınca hazırlık skoruna eklenir">+<?= (int) ($ri['weight'] ?? 0) ?></em><?php endif; ?></div><?php if (!$ri['ok']): ?><a title="<?= htmlspecialchars($linkTooltips[$ri['key']] ?? '') ?>" href="<?= htmlspecialchars($missLinks[$ri['key']] ?? $editBase) ?>">Doldur →</a> <button type="button" class="ical-copy-btn" data-copy="<?= htmlspecialchars($missLinks[$ri['key']] ?? $editBase) ?>" title="URL'yi kopyala">⧉ Kopyala</button><?php elseif ($riWarn): ?><a title="<?= htmlspecialchars($linkTooltips[$ri['key']] ?? '') ?>" href="<?= htmlspecialchars($warnLinks[$ri['key']] ?? '/nexustraveltech/tedarikci/ical-takvimler') ?>">İncele →</a> <button type="button" class="ical-copy-btn" data-copy="<?= htmlspecialchars($warnLinks[$ri['key']] ?? '/nexustraveltech/tedarikci/ical-takvimler') ?>" title="URL'yi kopyala">⧉ Kopyala</button><?php else: ?><a class="readiness-all-view" title="<?= htmlspecialchars($linkTooltips[$ri['key']] ?? '') ?>" href="<?= htmlspecialchars($missLinks[$ri['key']] ?? $editBase) ?>">Gör →</a> <button type="button" class="ical-copy-btn" data-copy="<?= htmlspecialchars($missLinks[$ri['key']] ?? $editBase) ?>" title="URL'yi kopyala">⧉ Kopyala</button><?php if (!$ri['ok'] && isset($secTitles[$ri['key']], $secAnchor[$ri['key']])): ?> <em class="readiness-sec-hint"><?= htmlspecialchars($secTitles[$ri['key']] . ' ' . $secAnchor[$ri['key']]) ?></em><?php endif; ?><?php endif; ?><?php if ($ri['key'] === 'media' && $ri['ok']): $mp = $pdo->prepare("SELECT file_path, is_cover FROM property_media WHERE property_id=? ORDER BY is_cover DESC, sort_order, id LIMIT 8"); $mp->execute([(int) $item['id']]); $mediaPrev = $mp->fetchAll(); if ($mediaPrev): ?><span class="readiness-media-preview" style="display:flex;flex-wrap:wrap;gap:6px;margin-top:8px;align-items:center"><?php foreach ($mediaPrev as $mv): $src = '/nexustraveltech/' . ltrim((string) $mv['file_path'], '/'); ?><img src="<?= htmlspecialchars($src) ?>" alt="" loading="lazy" style="width:56px;height:56px;object-fit:cover;border-radius:6px;border:1px solid #e1e5de"<?= $mv['is_cover'] ? ' title="Kapak" style="width:56px;height:56px;object-fit:cover;border-radius:6px;border:2px solid #b26a00"' : '' ?>><?php endforeach; ?></span><a class="readiness-media-more" href="<?= htmlspecialchars($missLinks['media'] ?? $editBase) ?>" style="font-size:11px;color:#4a6d8c;text-decoration:none;border-bottom:1px dotted #4a6d8c">Tümü →</a></span><?php endif; endif; ?></div>
            <?php endforeach; ?>
        </div>
    </div>
