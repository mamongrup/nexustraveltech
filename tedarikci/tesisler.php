...
<article class="property-card">
    ...
    <div class="readiness-block">
        ...
        <?php endif; ?>
        <?php if ($missing): ?>
        <ul class="readiness-missing"><?php foreach ($missing as $m): ?><li><a class="readiness-miss-link" title="<?= htmlspecialchars($linkTooltips[$m['key']] ?? '') ?>" href="<?= htmlspecialchars($missLinks[$m['key']] ?? $editBase) ?>">✗ <?= htmlspecialchars($m['label']) ?><?php if ($m['key'] === 'rules'): ?> <em>(opsiyonel)</em><?php else: ?> <em class="readiness-gain" title="Bu kalem tamamlanınca hazırlık skoruna eklenir">+<?= (int) ($m['weight'] ?? 0) ?></em><?php endif; ?><?php if (isset($secTitles[$m['key']], $secAnchor[$m['key']])): ?> <em class="readiness-sec-hint"><?= htmlspecialchars($secTitles[$m['key']] . ' ' . $secAnchor[$m['key']]) ?></em><?php endif; ?> →</a></li><?php endforeach; ?></ul>
        <?php elseif (!$warnings): ?>
